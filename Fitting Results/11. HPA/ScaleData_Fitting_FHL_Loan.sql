...
--    coupon,
    ...
    burnoutBucket = (case
                  when burnout < 100 THEN '100'
                  when burnout < 200 THEN '200'
                  when burnout < 400 THEN '400'
                  when burnout < 600 THEN '600'
                  when burnout < 800 THEN '800'
                  when burnout < 1000 THEN '1000'
    ...
    hpaBucket = (case
                  when HPA < 0 then 0
                  when HPA >= 10 then 12
                  else convert(int, HPA / 2) * 2 + 2 end),
--    satoBucket = (case
...
--                  else '100' end),
    ...
    sum(refi_incentive * currentBalance) / sum(currentBalance) as incentive,
    ...
    sum(secondLien * currentBalance) / sum(currentBalance) as pct_second_lien,
    ...
    sum(HPA * currentBalance) / sum(currentBalance) as hpa_,
...
--and occType <> 'INV'
GROUP BY asOf, marketTicker, highOLTV, occType, HARP_eligible, loanPurposeType, walaBucket, monthBucket, incentiveBucket, media_refiBucket, credit_availabilityBucket, burnoutBucket, waclsBucket, waolsBucket, cltvBucket, ficoBucket, HPABucket
ORDER BY asOf
;
COMMIT;
...
convert(numeric(10, 4), hpa_) as hpa,
