...
--DROP TABLE IF EXISTS #mtgRate_Conv;
--select --top 2
--    AsOfDate,
...
---- Creates the GNMA WAC Spread
--DROP TABLE IF EXISTS #GNM_Spread_Monthly;
--SELECT
--    AsOfDate,
--    SeriesNumValue as spread
--INTO #GNM_Spread_Monthly
--FROM report.TimeSeries ts
--JOIN report.TimeSeriesMeta tsm
--    ON ts.TimeSeriesMetaId = tsm.TimeSeriesMetaId
--WHERE 1=1
--    AND SeriesType = 'WAC_SPREAD'
--	AND ModelType = 'ScaleMtgRateModel v4.0'
--    AND TickerName = 'GINNIE_30YR'
--;
--COMMIT;
...
--    c.AsOf,
--    c.lastmonth,
--    c.SeriesNumValue as conv_mtg_rate,
--    c.SeriesNumValue + g.spread as gnma_mtg_rate
--into #mtgRate
--FROM #mtgRate_Conv c
--JOIN #GNM_Spread_Monthly g ON c.lastmonth = g.asofdate
--;
...
--SELECT 
--    t.asOf,
--    refinanceable = sum(case when (cl.origNoteRate + origPMI/100) - (conv_mtg_rate + currLLPA/400 + currPMI/100) - fixed_cost/100 >= 0.25 then calcScham 
--                             when (cl.origNoteRate + origPMI/100) - (gnma_mtg_rate + currMIP/100) - fixed_cost/100 >= 0.25 then calcScham
--                             else 0 end)/sum(case when calcScham is not NULL THEN calcScham ELSE 0.000001 END) 
--INTO #refiUniverse
--FROM scale.FHL_loanIncentive sli
--JOIN scale.fhl_Loan_Dev sl ON  sli.loanSeqNum = sl.loanSeqNum
--JOIN fhl.PIV_loan cl ON sli.loanSeqNum = cl.loanSeqNum
--JOIN #mtgRate t on t.lastmonth = sli.asOf
--JOIN fhl.PIV_loanHist clh on clh.loanSeqNum = sli.loanSeqNum AND clh.asof = sli.asof
--WHERE sli.version = '9.98'
--group by t.asOf
--order by t.asOf desc
--;
--COMMIT;
...
--DROP TABLE IF EXISTS #MediaEffect_Step1;
--SELECT 
--    convert(date, dateadd(month, +1, asof)) as asOf_lag1,
--    convert(date, dateadd(month, +2, asof)) as asOf_lag2,
--    convert(numeric(6,2), refinanceable * 100) as media_effect
--INTO #MediaEffect_Step1
--FROM #refiUniverse;
...
--    and slh.cltv <= 80
--    and wam > 180
    and clh.loanAge > 12 and clh.loanAge <= 48 
...
--    and cl.state not in ('NY','IL')
    and clh.asof >= '20160501'
--    and clh.asof <= '20161201'
--    and clh.asof >= '20120101'
--    and clh.asof <= '20130601'
--    and clh.asof >= '20170401'
    and clh.asof <= '20180601'
    and sl.origFICO >= 740
    and secondLien = 0
...
convert(numeric(8, 1), refi_incentive_) as refi_incentive,
