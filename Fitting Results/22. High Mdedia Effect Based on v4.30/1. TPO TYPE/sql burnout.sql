...
----version 3
insert into #refi values ( -9999, -150, 1 )
insert into #refi values ( -150, -125, 1 )
insert into #refi values ( -125, -100, 1 )
insert into #refi values ( -100, -75, 1 )
insert into #refi values ( -75, -50, 0.5 )
insert into #refi values ( -50, -25, 0.5 )
insert into #refi values ( -25, -20, 0.51 )
insert into #refi values ( -20, -15, 0.52 )
insert into #refi values ( -15, -10, 0.53 )
insert into #refi values ( -10, -5, 0.54 )
insert into #refi values ( -25, 0, 0.55 )
insert into #refi values ( 0, 5, 0.6 )
insert into #refi values ( 5, 10, 0.65 )
insert into #refi values ( 10, 15, 0.7 )
insert into #refi values ( 15, 20, 0.75 )
insert into #refi values ( 0, 25, 0.8 )
insert into #refi values ( 25, 30, 0.85 )
insert into #refi values ( 30, 35, 0.9 )
insert into #refi values ( 35, 40, 0.95 )
insert into #refi values ( 40, 45, 1 )
insert into #refi values ( 25, 50, 1.05 )
insert into #refi values ( 50, 55, 1.1 )
insert into #refi values ( 55, 60, 1.1 )
insert into #refi values ( 60, 65, 1.1 )
insert into #refi values ( 65, 70, 1.1 )
insert into #refi values ( 50, 75, 1.1 )
insert into #refi values ( 75, 100, 1.02 )
insert into #refi values ( 100, 125, 0.95 )
insert into #refi values ( 125, 150, 0.92 )
insert into #refi values ( 150, 175, 0.9 )
insert into #refi values ( 175, 200, 0.9 )
insert into #refi values ( 200, 225, 0.9 )
insert into #refi values ( 225, 250, 0.9 )
insert into #refi values ( 250, 9999, 0.9 )
...
,curve = substring( groupByColumn,charindex(':',groupByColumn)+1)  
    ...
    ,PCT_NonCashWindow = sum(((CASE WHEN tpoType in ('BROKER', 'CORRES', 'TPO') THEN 0 else 100 end) 
                            ...
                            THEN (CASE WHEN isCashWindow = 'Y' THEN 100 ELSE 0 END) ELSE (CASE WHEN tpoType in ('BROKER', 'CORRES', 'TPO') THEN 0 else 100 end) END))*schamBalance) / sum( 1*schamBalance)
    ,_Burnout_Fixed = sum( case when burnout_fixed_cost >-99999 then 1*schamBalance else NULL end *burnout_fixed_cost) / sum(case when burnout_fixed_cost >-99999   then  1*schamBalance  else NULL end)
                    from
                        FHL.PIV_LoanView l, scale.LoanPrepayTracking md1 , scale.fhl_loanIncentive i
                    join scale.fhl_loanburnout b on b.loanseqNum = i.loanseqNum and b.asof = i.asof and b.version = '2.70'
						where
                        ...
                        and i.version = '2.70'
                        and md1.asOf =  l.asOf 
                        and md1.modelId = '97' ------'97 is low media effect; 96 is high media
...
and (l.asOf >= '20160401' and l.asOf<='20161201')
...
and burnout_fixed_cost >6000 and burnout_fixed_cost <= 8000

-- and refi_incentive_eligible_fixed_cost>=-99999999 and refi_incentive_eligible_fixed_cost<=99999999
 and agency in ( 'FHL')
--and l.state not in ('NY','PR')
--and l.state not in ('AK','AL','AR','CT','DE','IA','LA','MD','ME','MS','ND','NJ','NM','OK','PA','VA','VT','WV','WY')
--and l.state not in ('AZ', 'CA','CO','DC','FL','GA','ID','MI','NV','OR','TX','UT','WA')
                    group by 
