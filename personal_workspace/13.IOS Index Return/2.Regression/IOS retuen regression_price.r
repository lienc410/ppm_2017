...
rawData <- read.csv("IOS Return Input Data 1m lag.csv")
...
# pairs(rawData[c("Price", "Px.Coupon","Carry","RealizedVol","PrepaymentRisk","YieldCurve")])


#regression, exam the result
xy.lm <- lm(rawData$Price ~ rawData$OAS + rawData$Carry + rawData$PrepaymentRisk + rawData$RealizedVol  + rawData$YieldCurve + rawData$LIBOR)
