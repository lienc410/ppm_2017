...
VALUES (getdate(),'FNM_Pool_Step 2_Create_Distribution_Table','START');
COMMIT;
...
UPDATE #FNM_PoolDistribution SET Pct_HARPed = 0.0 WHERE Pct_HARPed IS NULL;
COMMIT;
...
DROP TABLE IF EXISTS #FNM_OriginationDate_Ticker_OCC;
SELECT
...
INTO #FNM_OriginationDate_Ticker_OCC
...
FROM #FNM_OriginationDate_Ticker_OCC occ
...
-- USE First Available Data (1986-01-01) for missing origination period

DROP TABLE IF EXISTS #FNM_OriginationDate_First_Origination_OCC;
SELECT
    min(originationDate) as first_origination,
    marketTicker,
    sum(balance * Pct_OWNER) / sum(balance) as WAVG_OWNER,
    sum(balance * Pct_2ND) / sum(balance) as WAVG_2ND,
    sum(balance * Pct_INV) / sum(balance) as WAVG_INV,
    sum(balance) as sum_ls
INTO #FNM_OriginationDate_First_Origination_OCC
FROM #FNM_PoolDistribution
WHERE Pct_OWNER IS NOT NULL
...
UPDATE #FNM_PoolDistribution perf SET perf.Pct_OWNER = occ.WAVG_OWNER, perf.Pct_2ND = occ.WAVG_2ND, perf.Pct_INV = occ.WAVG_INV
FROM #FNM_OriginationDate_First_Origination_OCC occ
WHERE perf.Pct_OWNER IS NULL
;
COMMIT;


DELETE FROM #FNM_PoolDistribution where Pct_OWNER is NULL or Pct_2ND is NULL or Pct_INV  is NULL;
...
DROP TABLE IF EXISTS #FNM_OriginationDate_Ticker_PURP;
SELECT
    year(originationDate) as origYear,
    marketTicker,
    ...
    sum(balance) as sum_ls
INTO #FNM_OriginationDate_Ticker_PURP
FROM #FNM_PoolDistribution
...
FROM #FNM_OriginationDate_Ticker_PURP purp
...
-- USE First Available Data (1979-02-01 for FNCI and 1966-01-01 for FNCT) for missing origination period

DROP TABLE IF EXISTS #FNM_OriginationDate_First_Origination_PURP;
SELECT
    min(originationDate) as first_origination,
    marketTicker,
    sum(balance * Pct_PURCH) / sum(balance) as WAVG_PURCH,
    sum(balance * Pct_REFI) / sum(balance) as WAVG_REFI,
    sum(balance) as sum_ls
INTO #FNM_OriginationDate_First_Origination_PURP
FROM #FNM_PoolDistribution
WHERE Pct_PURCH IS NOT NULL AND Pct_REFI IS NOT NULL
GROUP BY marketTicker
;
...
UPDATE #FNM_PoolDistribution perf SET perf.Pct_PURCH = purp.WAVG_PURCH, perf.Pct_REFI = purp.WAVG_REFI
FROM #FNM_OriginationDate_First_Origination_PURP purp
WHERE perf.marketTicker = purp.marketTicker
        AND (perf.Pct_PURCH IS NULL OR perf.Pct_REFI IS NULL)
;
COMMIT;

DELETE FROM #FNM_PoolDistribution WHERE Pct_PURCH is NULL or Pct_REFI is NULL;
...
DROP TABLE IF EXISTS #FNM_OriginationDate_Ticker_CHAN;
SELECT
    marketTicker,
    year(originationDate) as origYear,
    ...
    sum(balance) as sum_ls
INTO #FNM_OriginationDate_Ticker_CHAN
FROM #FNM_PoolDistribution
...
FROM #FNM_OriginationDate_Ticker_CHAN chan
WHERE 1=1
    ...
    AND perf.marketTicker in ('FNCL', 'FNCT', 'FNCI');
;
COMMIT;
...
DROP TABLE IF EXISTS #FNM_OriginationDate_ticker_UNIT;
SELECT
    year(originationDate) as origYear,
    marketTicker,
    ...
    sum(balance) as sum_ls
INTO #FNM_OriginationDate_ticker_UNIT
FROM #FNM_PoolDistribution
...
FROM #FNM_OriginationDate_ticker_UNIT unit
...
DROP TABLE IF EXISTS #FNM_OriginationDate_First_Origination_UNIT;
SELECT
    min(originationDate) as first_origination,
    marketTicker,
    sum(balance * Pct_SINGLE_UNIT) / sum(balance) as WAVG_Single,
    sum(balance * Pct_MULTIPLE_UNIT) / sum(balance) as WAVG_Multiple,
    sum(balance) as sum_ls
INTO #FNM_OriginationDate_First_Origination_UNIT
FROM #FNM_PoolDistribution
WHERE Pct_SINGLE_UNIT IS NOT NULL AND Pct_MULTIPLE_UNIT IS NOT NULL
GROUP BY marketTicker
;
...
UPDATE #FNM_PoolDistribution perf SET perf.Pct_SINGLE_UNIT = unit.WAVG_Single, perf.Pct_MULTIPLE_UNIT = unit.WAVG_Multiple
FROM #FNM_OriginationDate_First_Origination_UNIT unit
WHERE perf.marketTicker = unit.marketTicker
        AND (perf.Pct_SINGLE_UNIT IS NULL OR perf.Pct_MULTIPLE_UNIT IS NULL)
;
COMMIT;
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 200)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Est_Pct_HARPed: %1!', @cnt
            RETURN
        END
...
--        if (@cnt > 200)
--        BEGIN
--            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_HARPed: %1!', @cnt
--            RETURN
--        END
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_MtgIns: %1!', @cnt
--            RETURN
--        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_OWNER: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_2ND: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_INV: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with Occupancy Type not summing to 100%: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_PURCH: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_REFI: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with Purpose Type not summing to 100%: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with Channel Type not summing to 100%: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_SINGLE_UNIT: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for Pct_MULTIPLE_UNIT: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with Units Type not summing to 100%: %1!', @cnt
            RETURN
        END
...
--	 FROM scale.FNM_PoolDistribution
...
INSERT INTO scale.JobStatus (EndTime, JobName, JobStatus, #LoansBeforeProcess, #LoansUpdated, #LoansAfterProcess)
    SELECT
     getdate(),
    'FNM_Pool_Step 2_Create_Distribution_Table',
    'SUCCESS', 
    @count_previous,
    @count_current,
