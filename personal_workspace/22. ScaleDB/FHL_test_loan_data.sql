...
SELECT count(1) FROM scale.FHL_Loan;
SELECT count(distinct loanSeqNum) FROM scale.FHL_Loan;

SELECT count(distinct loanSeqNum) FROM scale.FHL_LoanHist;
SELECT count(distinct loanSeqNum) FROM scale.FHL_LoanEligibility;
SELECT count(distinct loanSeqNum) FROM scale.FHL_LoanIncentive WHERE version = '2.00';
SELECT count(distinct loanSeqNum) FROM scale.FHL_LoanBurnout WHERE version = '2.00';

SELECT count(1) FROM scale.FHL_LoanHist;
SELECT count(1) FROM scale.FHL_LoanEligibility;
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE version = '2.00';
SELECT count(1) FROM scale.FHL_LoanBurnout WHERE version = '2.00';

SELECT version, count(1) FROM scale.FHL_LoanIncentive GROUP BY version;
SELECT version, count(1) FROM scale.FHL_LoanBurnout GROUP BY version;


-- Uniqueness Tests (Should all be 0 rows)

SELECT loanSeqNum, asOf, count(1) as cnt FROM scale.FHL_LoanHist GROUP BY loanSeqNum, asOf HAVING cnt > 1;
SELECT loanSeqNum, asOf, count(1) as cnt FROM scale.FHL_LoanEligibility GROUP BY loanSeqNum, asOf HAVING cnt > 1;
SELECT loanSeqNum, asOf, count(1) as cnt FROM scale.FHL_LoanIncentive WHERE version = '2.00' GROUP BY loanSeqNum, asOf HAVING cnt > 1;
SELECT loanSeqNum, asOf, count(1) as cnt FROM scale.FHL_LoanBurnout WHERE version = '2.00' GROUP BY loanSeqNum, asOf HAVING cnt > 1;


-- Null and Data Validity Tests (Should all be 0 counts)

SELECT count(1) FROM scale.FHL_Loan WHERE origLoanSize IS NULL OR origLoanSize <= 0.0 OR origLoanSize > 10000000
SELECT count(1) FROM scale.FHL_Loan WHERE numberUnits IS NULL OR numberUnits <= 0 OR numberUnits > 10
SELECT count(1) FROM scale.FHL_Loan WHERE origFICO IS NULL OR origFICO <= 0.0 OR origFICO > 900
SELECT count(1) FROM scale.FHL_Loan WHERE origLTV IS NULL OR origLTV <= 0.0 OR origLTV > 1000
SELECT count(1) FROM scale.FHL_Loan WHERE origNoteRate IS NULL OR origNoteRate <= 0.0 OR origNoteRate > 20
SELECT count(1) FROM scale.FHL_Loan WHERE miLevel IS NULL OR miLevel < 0.0 OR miLevel > 80
SELECT count(1) FROM scale.FHL_Loan WHERE HPI_orig IS NULL OR HPI_orig < 0.0 OR HPI_orig > 1000
SELECT count(1) FROM scale.FHL_LoanHist WHERE cltv IS NULL OR cltv <= 0.0 OR cltv > 1000
SELECT count(1) FROM scale.FHL_LoanHist WHERE hpa IS NULL OR hpa < 0.5 OR hpa > 10

SELECT count(1) FROM scale.FHL_Loan WHERE origPMI IS NULL
SELECT count(1) FROM scale.FHL_Loan WHERE origPMI < 0.0 OR origPMI > 500.0

SELECT count(1) FROM scale.FHL_Loan WHERE percentHARPed IS NULL
SELECT count(1) FROM scale.FHL_Loan WHERE percentHARPed < 0.0 OR percentHARPed > 100

SELECT count(1) FROM scale.FHL_LoanEligibility WHERE HARP_eligible IS NULL 
SELECT count(1) FROM scale.FHL_LoanEligibility WHERE conventional_eligible IS NULL
SELECT count(1) FROM scale.FHL_LoanEligibility WHERE fha_eligible IS NULL
SELECT count(1) FROM scale.FHL_LoanEligibility WHERE HARP_eligible < 0.0 OR HARP_eligible > 1
SELECT count(1) FROM scale.FHL_LoanEligibility WHERE conventional_eligible < 0.0 OR conventional_eligible > 1
SELECT count(1) FROM scale.FHL_LoanEligibility WHERE fha_eligible < 0.0 OR fha_eligible > 1

SELECT count(1) FROM scale.FHL_LoanIncentive WHERE conventional_refi_incentive IS NULL
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE fha_refi_incentive IS NULL
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE currLLPA IS NULL
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE currPMI IS NULL
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE currMIP IS NULL
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE conventional_refi_incentive < -2000.0 OR conventional_refi_incentive > 2000.0
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE fha_refi_incentive < -2000.0 OR fha_refi_incentive > 2000.0
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE currLLPA < -100 OR currLLPA > 2000.0
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE currPMI < 0 OR currPMI > 1000.0
SELECT count(1) FROM scale.FHL_LoanIncentive WHERE currMIP < 0 OR currMIP > 500.0
...
SELECT sl.marketTicker, sl.originationDate, count(1), sum(schamBalance), sum(schamBalance * burnout) / sum(CASE WHEN burnout IS NULL THEN 0.0 ELSE schamBalance END) as wavg_burnout FROM scale.FHL_Loan sl JOIN scale.FHL_LoanBurnout sli ON sl.loanSeqNum = sli.loanSeqNum JOIN scale.FHL_LoanHist slh ON slh.loanSeqNum = sli.loanSeqNum AND slh.asOf = sli.asOf WHERE version = '2.10' GROUP BY sl.marketTicker, sl.originationDate ORDER BY sl.originationDate
SELECT sl.marketTicker, slh.asOf, count(1), sum(schamBalance), sum(schamBalance * burnout) / sum(CASE WHEN burnout IS NULL THEN 0.0 ELSE schamBalance END) as wavg_burnout FROM scale.FHL_Loan sl JOIN scale.FHL_LoanBurnout sli ON sl.loanSeqNum = sli.loanSeqNum JOIN scale.FHL_LoanHist slh ON slh.loanSeqNum = sli.loanSeqNum AND slh.asOf = sli.asOf WHERE version = '2.10' GROUP BY sl.marketTicker, slh.asOf ORDER BY slh.asOf
...
SELECT sl.marketTicker, slh.asOf, count(1), sum(schamBalance), sum(schamBalance * burnout) / sum(CASE WHEN burnout IS NULL THEN 0.0 ELSE schamBalance END) as wavg_burnout FROM scale.FHL_Loan sl JOIN scale.FHL_LoanBurnout sli ON sl.loanSeqNum = sli.loanSeqNum JOIN scale.FHL_LoanHist slh ON slh.loanSeqNum = sli.loanSeqNum AND slh.asOf = sli.asOf WHERE version = '2.00' GROUP BY sl.marketTicker, slh.asOf ORDER BY slh.asOf

SELECT sl.marketTicker, sl.originationDate, count(1), sum(schamBalance), sum(schamBalance * burnout) / sum(CASE WHEN burnout IS NULL THEN 0.0 ELSE schamBalance END) as wavg_burnout FROM scale.FHL_Loan sl JOIN scale.FHL_LoanBurnout sli ON sl.loanSeqNum = sli.loanSeqNum JOIN scale.FHL_LoanHist slh ON slh.loanSeqNum = sli.loanSeqNum AND slh.asOf = sli.asOf WHERE version = '1.00' GROUP BY sl.marketTicker, sl.originationDate ORDER BY sl.originationDate
SELECT sl.marketTicker, slh.asOf, count(1), sum(schamBalance), sum(schamBalance * burnout) / sum(CASE WHEN burnout IS NULL THEN 0.0 ELSE schamBalance END) as wavg_burnout FROM scale.FHL_Loan sl JOIN scale.FHL_LoanBurnout sli ON sl.loanSeqNum = sli.loanSeqNum JOIN scale.FHL_LoanHist slh ON slh.loanSeqNum = sli.loanSeqNum AND slh.asOf = sli.asOf WHERE version = '1.00' GROUP BY sl.marketTicker, slh.asOf ORDER BY slh.asOf

SELECT version, slh.asOf, count(1), sum(schamBalance), sum(schamBalance * burnout) / sum(CASE WHEN burnout IS NULL THEN 0.0 ELSE schamBalance END) as wavg_burnout FROM scale.FHL_Loan sl JOIN scale.FHL_LoanBurnout sli ON sl.loanSeqNum = sli.loanSeqNum JOIN scale.FHL_LoanHist slh ON slh.loanSeqNum = sli.loanSeqNum AND slh.asOf = sli.asOf WHERE sl.marketTicker IN ('FGLMC') GROUP BY version, slh.asOf ORDER BY version, slh.asOf
