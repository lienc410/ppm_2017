...
VALUES (getdate(),'FNM_Pool_Step 6_Create_Pool_Burnout_Table','START');
COMMIT;
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with NULL Value for Burnout: %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FNM_PoolBurnout WHERE burnout < 0.0;

        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with INVALID Value for Burnout: %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
INSERT INTO scale.JobStatus (EndTime, JobName, JobStatus, #LoansBeforeProcess, #LoansUpdated, #LoansAfterProcess)
    SELECT
     getdate(),
    'FNM_Pool_Step 6_Create_Pool_Burnout_Table',
    'SUCCESS', 
    @count_previous,
    @count_current,
