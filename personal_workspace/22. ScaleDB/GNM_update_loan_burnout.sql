-- Ginnie
-- Loan Level
-- Part 6 of 6
-- Script to update Burnout
-- Updates Burnout


INSERT INTO scale.jobstatus(StartTime,JobName,JobStatus)
VALUES (getdate(),'GNM_Loan_Step 6_Create_Loan_Burnout_Table','START');
COMMIT;


-- Before execute the script, need to set the dates to run for
DROP TABLE IF EXISTS #tmp_asOf;
SELECT CAST('#####ASOF#####' AS DATE) asOf INTO #tmp_asOf
--SELECT CAST ('2018-03-01' AS DATE) asOf INTO #tmp_asOf
;

-- Before execute the script, need to set the appType to run for
DROP TABLE IF EXISTS #tmp_appType;
SELECT '#####APPTYPE#####' AS appType INTO #tmp_appType
--SELECT '9.99' AS appType INTO #tmp_appType
;

-- Before execute the script, need to set the tickers
DROP TABLE IF EXISTS #ticker;
CREATE TABLE #ticker(
    tickerName varchar(20),
    mtgRateSeries varchar(30)
)
;
INSERT #ticker SELECT 'GNSF', 'GINNIE_30YR';   --Ginnie
INSERT #ticker SELECT 'G2SF', 'GINNIE_30YR';   --Ginnie
INSERT #ticker SELECT 'G2SF-JM', 'GINNIE_30YR';-- Ginnie Jumbo
COMMIT;

CREATE LF INDEX loan_tickerName_idx ON #ticker(tickerName);
COMMIT;

-- Before execute the script, need to set the version to run for
DROP TABLE IF EXISTS #tmp_version;
SELECT mvc.tickerName, mvc.appType, mvc.burnoutVersion_LL as burnoutVersion, mvc.incentiveVersion_LL as incentiveVersion, mvc.originationVersion_LL as originationVersion  
INTO #tmp_version
FROM scale.ModelVersionConfig mvc
JOIN #tmp_appType tat
     ON mvc.appType = tat.appType
JOIN #ticker tk
     ON mvc.tickerName = tk.tickerName
;
COMMIT;



-- Initial Burnout computation

DROP TABLE IF EXISTS #GNM_LoanBurnout;

CREATE TABLE #GNM_LoanBurnout(
marketTicker  char(10), 
version char(10), 
loanSeqNum char(25), 
asOf date, 
burnout numeric(10,4),
burnout_fixed_cost numeric(10,4));
COMMIT;

declare @boundary date
set @boundary='1978-12-31'

WHILE @boundary<=dateadd(year,+1,getdate())
BEGIN


INSERT INTO #GNM_LoanBurnout
SELECT
    sl.marketTicker, 
	tpv.burnoutVersion as version,
    t0.loanSeqNum,
    t0.asOf as asOf,
    sum(CASE WHEN t1.refi_incentive_eligible < 0.0 THEN 0.0
             ELSE CASE WHEN t1.refi_incentive_eligible > 200.0 THEN 200.0
             ELSE t1.refi_incentive_eligible END END
        ) as burnout,
    sum(CASE WHEN t1.refi_incentive_eligible_fixed_cost < 0.0 THEN 0.0
             ELSE CASE WHEN t1.refi_incentive_eligible_fixed_cost > 200.0 THEN 200.0
             ELSE t1.refi_incentive_eligible_fixed_cost END END
        ) as burnout_fixed_cost
FROM scale.GNM_Loan_dev sl
JOIN #ticker tk
    ON sl.marketTicker = tk.tickerName
JOIN #tmp_version tpv
    ON sl.marketTicker = tpv.tickerName
JOIN scale.GNM_LoanIncentive t0
    ON sl.loanSeqNum = t0.loanSeqNum
JOIN scale.GNM_LoanIncentive t1
    ON t0.loanSeqNum = t1.loanSeqNum
    AND t0.asOf >= t1.asOf
WHERE 1=1
    AND t0.version in (select incentiveVersion from #tmp_version)
    AND t1.version in (select incentiveVersion from #tmp_version)
    AND sl.marketTicker in (SELECT tickerName FROM #ticker)
    AND sl.version in (select originationVersion from #tmp_version)
	AND sl.originationdate<=@boundary
	AND sl.originationdate>dateadd(year,-1,@boundary)
GROUP BY sl.marketTicker, t0.loanSeqNum, t0.asOf, version

set @boundary=dateadd(year,1,@boundary)

END;


    

-- Tests

--SELECT l.marketTicker, sl.originationDate, count(1), sum(balance), sum(balance * l.burnout) / sum(CASE WHEN l.burnout IS NULL THEN 0.0 ELSE balance END) as wavg_burnout FROM #GNM_LoanBurnout l JOIN scale.GNM_LoanHist lh ON l.loanSeqNum = lh.loanSeqNum AND l.asOf = lh.asOf JOIN scale.GNM_Loan sl ON sl.loanSeqNum = lh.loanSeqNum GROUP BY l.marketTicker, sl.originationDate ORDER BY sl.originationDate
--SELECT l.marketTicker, l.asOf, count(1), sum(balance), sum(balance * l.burnout) / sum(CASE WHEN l.burnout IS NULL THEN 0.0 ELSE balance END) as wavg_burnout FROM #GNM_LoanBurnout l JOIN scale.GNM_LoanHist lh ON l.loanSeqNum = lh.loanSeqNum AND l.asOf = lh.asOf GROUP BY l.marketTicker, l.asOf ORDER BY l.asOf

----------------------------------------------------------------------------------------------
-- Check to see if Loans have burnout populated
----------------------------------------------------------------------------------------------
        declare   @cnt int
   --      SELECT
   --          @cnt =  count(1)
    --     FROM #GNM_LoanBurnout WHERE burnout IS NULL

  --       if (@cnt > 0)
  --       BEGIN
   --          RAISERROR 99999 'Number of GNM Loans with NULL Value for Burnout: %1!', @cnt
   --          RETURN
   --      END
----------------------------------------------------------------------------------------------
-- Check to see if burnout is in valid range
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM
            #GNM_LoanBurnout
        WHERE
            burnout < 0.0

        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of GNM Loans with INVALID Value for Burnout: %1!', @cnt
            RETURN
        END


----------------------------------------------------------------------------------------------
-- Check to see if new loans are loaded into database (need to be commented unless load new data)
----------------------------------------------------------------------------------------------
--     declare @count_previous int
--	 SELECT @count_previous = count(distinct(loanSeqNum))
--	 FROM scale.GNM_LoanBurnout
--  WHERE version IN (SELECT burnoutVersion FROM #tmp_version)

--	 declare @count_current int
--	 SELECT @count_current = count(distinct(loanSeqNum))
--	 FROM #GNM_LoanBurnout
	 
--	 if (@cnt_current - @cnt_previous <= 0 )
--        BEGIN
--           RAISERROR 99999 'There is no new loan loading into database for burnout table. Previous loan count: %1!; Current loan count: %2!', @cnt_previous, @cnt_current
--           RETURN
 --      END 
 
 ----------------------------------------------------------------------------------------------
-- Check to see if previous incentive is available for burnout
----------------------------------------------------------------------------------------------

--	 declare @count_check int
--	 SELECT @count_current = count(distinct(loanSeqNum))
--	 FROM #GNM_LoanBurnout
	 
--	 if (@cnt_check = 0 )
--        BEGIN
--           RAISERROR 99999 'There is not enough data to calculate burnout. Load more incentive!'
--           RETURN
 --      END 
 
 
 
-- End Tests



-- Load Data into Scale Burnout Table


DELETE FROM scale.GNM_LoanBurnout
FROM scale.GNM_LoanBurnout slb, #GNM_LoanBurnout lb
WHERE 1=1
    AND slb.loanSeqNum = lb.loanSeqNum
    AND slb.asOf = lb.asOf
    AND lb.marketTicker IN (SELECT tickerName FROM #ticker)
    AND slb.asOf >= (SELECT asOf from #tmp_asOf)
    AND slb.version in (SELECT burnoutVersion from #tmp_version)
;


INSERT INTO scale.GNM_LoanBurnout (loanSeqNum, asOf, version, burnout, burnout_fixed_cost)
SELECT
    loanSeqNum,
    asOf,
    version,
    burnout,
	burnout_fixed_cost
FROM #GNM_LoanBurnout
WHERE marketTicker IN (SELECT tickerName FROM #ticker)
    AND asOf >= (SELECT asOf from #tmp_asOf)
;

declare @count_previous int
   
    SELECT @count_previous = count(distinct(loanSeqNum))
    FROM scale.GNM_LoanBurnout
	WHERE version IN (SELECT burnoutVersion FROM #tmp_version)
	
declare @count_current int
	 
	 SELECT @count_current = count(distinct(loanSeqNum))
	 FROM #GNM_LoanBurnout
	 
INSERT INTO scale.JobStatus (EndTime, JobName, JobStatus, #LoansBeforeProcess, #LoansUpdated, #LoansAfterProcess)
    SELECT
     getdate(),
    'GNM_Loan_Step 6_Create_Loan_Burnout_Table', 
    'SUCCESS', 
    @count_previous,
    @count_current,
	CASE WHEN(@count_previous>@count_current) THEN @count_previous ELSE @count_current END
;
COMMIT;
