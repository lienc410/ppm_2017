...
--Remove the loans already available in the missing hist table
Delete from #FNM_missing_hist_list
where loanSeqNum in (Select loanSeqNum from scale.fnm_missing_hist);
commit;
...
UPDATE #LoanIncentive inc SET conv_incentive = CASE WHEN inc.marketTicker = 'FNCT' THEN origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 3.5) - pmi
                                                    ELSE origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 4.0) - pmi END;
delete from #LoanIncentive where conv_incentive is NULL;

UPDATE #LoanIncentive inc SET conv_incentive_fc = CASE WHEN inc.marketTicker = 'FNCT' THEN origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 3.5) - pmi - fixed_cost
...
UPDATE #LoanIncentive inc SET fha_incentive = origNoteRate + (origPMI / 100.0) - MtgRate_GNMA - mip;
UPDATE #LoanIncentive inc SET fha_incentive = 0 WHERE fha_incentive is NULL;

UPDATE #LoanIncentive inc SET fha_incentive_fc = origNoteRate + (origPMI / 100.0) - MtgRate_GNMA - mip - fixed_cost;
