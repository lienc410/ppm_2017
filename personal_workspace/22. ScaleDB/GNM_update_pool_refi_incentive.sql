...
------------------------------------------------------------
DROP TABLE IF EXISTS #GNM_LoanBasedIncentive;
SELECT
    pi.issueId,
    pi.asOf,
    sum(sl.origMIP * slh.schamBalance) / sum(CASE WHEN sl.origMIP IS NULL THEN 0.0 ELSE slh.schamBalance END) as mip_begin,
    sum(sli.currPMI * slh.schamBalance) / sum(CASE WHEN sli.currPMI IS NULL THEN 0.0 ELSE slh.schamBalance END) as pmi,
    sum(sli.currLLPA * slh.schamBalance) / sum(CASE WHEN sli.currLLPA IS NULL THEN 0.0 ELSE slh.schamBalance END) as llpa,
    sum(sli.currMIP * slh.schamBalance) / sum(CASE WHEN sli.currMIP IS NULL THEN 0.0 ELSE slh.schamBalance END) as mip,
    sum(sli.conventional_refi_incentive * slh.schamBalance) / sum(CASE WHEN sli.conventional_refi_incentive IS NULL THEN 0.0 ELSE slh.schamBalance END) as conventional_refi_incentive,
    sum(sli.fha_refi_incentive * slh.schamBalance) / sum(CASE WHEN sli.fha_refi_incentive IS NULL THEN 0.0 ELSE slh.schamBalance END) as fha_refi_incentive,
    sum((CASE WHEN sli.conventional_refi_incentive > sli.fha_refi_incentive THEN sli.conventional_refi_incentive ELSE sli.fha_refi_incentive END) * slh.schamBalance) / sum(slh.schamBalance) as refi_incentive,
    sum(sli.refi_incentive_eligible * slh.schamBalance) / sum(CASE WHEN sli.refi_incentive_eligible IS NULL THEN 0.0 ELSE slh.schamBalance END) as refi_incentive_eligible
INTO #GNM_LoanBasedIncentive
FROM #GNM_PoolIncentive pi
...
FROM #GNM_LoanBasedIncentive lbi
...
-- Adjust mip_begin for %FHA
UPDATE #GNM_PoolBasedIncentive inc SET inc.mip_begin = inc.mip_begin * (pctFHA / 100.0);
COMMIT;
...
UPDATE #GNM_PoolBasedIncentive inc SET
    conventional_refi_incentive = 100.0 * (wac + (mip_begin / 100.0) - MtgRate - (llpa / 400.0) - (pmi / 100.0)),
...
DROP TABLE IF EXISTS #GNM_IncentiveBias;
SELECT
    CASE
...
INTO #GNM_IncentiveBias
FROM #GNM_LoanBasedIncentive
WHERE 1=1
...
DROP TABLE IF EXISTS #GNM_Buckets;
SELECT
    issueId,
    asOf,
    CASE
    ...
    END as refi_bucket
INTO #GNM_Buckets
FROM #GNM_PoolBasedIncentive
;

UPDATE #GNM_PoolBasedIncentive inc SET
    inc.refi_incentive_eligible = inc.refi_incentive + ib.avg_diff
FROM #GNM_IncentiveBias ib, #GNM_Buckets bu
WHERE 1=1
...
DELETE FROM scale.GNM_PoolIncentive
FROM scale.GNM_PoolIncentive spi, #GNM_PoolIncentive pi
WHERE 1=1
...
INSERT INTO scale.GNM_PoolIncentive (issueId, asOf, version, conventional_refi_incentive, fha_refi_incentive, refi_incentive, refi_incentive_eligible, origMIP, currLLPA, currPMI, currMIP)
SELECT
    issueId,
    asOf,
    @version,
