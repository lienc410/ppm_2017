	------------------------------------------------------------------------
	------------------procedure for the loan level model with Lowest Rate
	------------------------------------------------------------------------
	--alter procedure Lien_Test (@asof Date) 
	alter procedure RefinancibleUniverse_LoanLevel_withLowestRate (@asof Date) 
	as
	begin
	...
	select asOfDate,
		   convert(date,DateAdd(month,1,convert(date, year(asOfDate) || '-' || month(asOfDate) || '-01'))) as AsOfMonth1,
		   lastmonth = dateadd(month, -1, AsOfMonth1),
		   AsOfMonth = lastmonth,
...
order by AsOfDate DESC
	...
	exec RefinancibleUniverse_LoanLevel_withLowestRate '2015-06-25'