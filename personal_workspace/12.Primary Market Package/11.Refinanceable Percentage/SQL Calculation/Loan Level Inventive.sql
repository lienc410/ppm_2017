...
select f.asofdate,f.FicoStart, f.FicoEnd, f.LtvStart, f.LtvEnd, o.occtype, u.numUnits, BaseLLTV = (case when f.LLPA is null then 99999 else f.LLPA end),
        a.LLPA as AMDC1,  aa.LLPA as AMDC2, AMDC = (case when AMDC1 is null then AMDC2 else AMDC1 end),
        HighLTV = (case when l.llpa is null then 99999 else l.llpa end), 
        Occp = (case when o.llpa is null then 99999 else o.llpa end), 
        UnitL = (case when u.llpa is null then 99999 else u.llpa end),
...
select 	t.asOfDate,
		refinanceible = sum(
							( case when 	
								( (case when l.calcorigmonth_LL <= 200906 then (case when tl.totalLLPA > 0.75 then 0.75 else tl.totalLLPA end) else tl.totalLLPA end)
								+ (case when l.calcorigmonth_LL <= 200906 then 0 else tp.PMI/100 end)
							...
							* (case when lh.currentRpb > 0 then lh.currentRpb  + pre.ITMBalance else 0.000001 end)
                         )
						/ sum (case when lh.currentRpb > 0 then lh.currentRpb  + pre.ITMBalance else 0.000001 end)
