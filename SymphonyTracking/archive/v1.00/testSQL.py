...
def generate_pool_data(full_update, model, factor_date):
    ...
    bat_path = os.path.join(symphony_tracking_path, 'bat')
    sql_path = os.path.join(symphony_tracking_path, 'sql')
    # ticker_dict = {'jumbo': ('jumbo_tickers.sql',
    ...
    bat_file = os.path.join(bat_path, 'ReportdbSQL.bat')
...
#    extract_path = os.path.join('G:/tempExtract/PrepayPoolData/Model_')
    ...
    for ticker, ticker_tuple in ticker_dict.iteritems():
        ...
        print ticker_sql_file  
        with open(ticker_sql_file, 'r') as inp:
		for line in inp:
				if (re.search(";", line)):
					full_sql = full_sql + line
					print("----BEG------")
					print(full_sql)
					print("-----END-----")
...
def generate_model_output(model, factor_date):
    ...
    scale_path = 'S:/IT/Dev/ScaleMQ/bobby_release/'
