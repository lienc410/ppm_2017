...
def generate_pool_data(full_update, model, factor_date):
    ...
    def process(ticker):
        ...
        vintage_list = ticker_tuple[2] if full_update else [1950, latest_year]
...
def load_model_data(model, factor_date, full_update):
    ...
    # Get Sybase IQ Database connection
    conn = pyodbc.connect('DSN=' + "ScaleAgencyPool" + ';UID=' + "scale"+';PWD=' + "xaYc14rJ" + ';DATABASE=' + 'Agency' + ';Autostop=No')
    cursor = conn.cursor()
    ...
    with open("sql/loadPoolPrepay.sql", 'r') as sqlFile:
    ...
    if not (full_update):
        with open("sql/updatePoolPrepay.sql", 'r') as sqlFile:
