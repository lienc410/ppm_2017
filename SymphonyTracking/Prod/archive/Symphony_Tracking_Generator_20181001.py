...
def generate_data(full_update, factor_date, sample_only):
    ...
    if 'ginnie' in running_ticker_list:
        ...
        for ticker in ginnie_list:
            generate_loan_data(ginnie_list, full_update, factor_date, sample_only)
        running_ticker_list.remove('ginnie')
    
    if len(running_ticker_list):
        generate_pool_data(running_ticker_list, full_update, factor_date, sample_only)
    
    print "Exiting generate_data"
    
def generate_model_output(factor_date, use_r_engine):
    ...
    if 'ginnie' in running_ticker_list:
        ginnie_list = ['ginnie']
        for ticker in ginnie_list:
            generate_model_output_loan(ginnie_list, factor_date, use_r_engine)
        running_ticker_list.remove('ginnie')
    
    if len(running_ticker_list):
        generate_model_output_pool(running_ticker_list, factor_date, use_r_engine)
    
    print "Exiting generate_model_output"
    
def load_model_data(factor_date, full_update):
    ...
    if 'ginnie' in running_ticker_list:
        ginnie_list = ['ginnie']
        for ticker in ginnie_list:
            load_model_data_loan(ginnie_list, factor_date, full_update)
        running_ticker_list.remove('ginnie')
    
    if len(running_ticker_list):
        load_model_data_pool(running_ticker_list, factor_date, full_update)
    
    print "Exiting load_model_data"

def generate_pool_data(run_ticker_list, full_update, factor_date, sample_only):
    ...
    def process(ticker):
        ...
        model_name = ticker_tuple['model_name']
        model_version = ticker_tuple['model_version']
        ...
        symphony_tracking_path = 'S:/IT/Production/Scripts/python/Scale/SymphonyTracking/' + version + '/'
...
def generate_loan_data(run_ticker_list, full_update, factor_date, sample_only):
...
def generate_model_output_pool(run_ticker_list, factor_date, use_r_engine):
...
def generate_model_output_loan(run_ticker_list, factor_date, use_r_engine):
...
def load_model_data_pool(run_ticker_list, factor_date, full_update):
    ...
    for run_ticker in run_ticker_list:
        ...
        cleanupSQLStmt = "delete " + tableName + "  where modelId = '" + model_version + "'" if full_update else " delete staging_PoolPrepayTracking"
...
def load_model_data_loan(run_ticker_list, factor_date, full_update):
    ...
    for run_ticker in run_ticker_list:
        ...
        cursor.execute(cleanupSQLStmt)
        cursor.commit()
        # Cleanup pool data in table
        print "Cleaning Pool Up Old Data"
        cleanupSQLStmt = "delete " + pool_tableName + "  where modelId = '" + model_version + "' or modelId is  NULL" if full_update else " delete staging_PoolPrepayTracking"
        cleanupSQLStmt = "delete " + pool_tableName + "  where " + " modelId is  NULL" if full_update else " delete staging_PoolPrepayTracking"
        print (cleanupSQLStmt)
        cursor.execute(cleanupSQLStmt)
        cursor.commit()
...
def usage(function_name):
    print("Usage 1: %s --factor_update --factor_date 20161201" % function_name)
    print("Usage 2: %s --new_model --factor_date 20161201" % function_name)
    print("Usage 3: %s --new_model --factor_date 20161201 -R" % function_name)
    print("Usage 4: %s --new_model --factor_date 20161201 -S" % function_name)
...
def main(argv):
    ...
    ticker_dict = {'ginnie': {'model_version': gnmandolin_model_version,
                              'model_name': 'Gnmandolin_v' + gnmandolin_model_version,
                              'model_type': 'GINNIE',
                              'r_path': 'S:\\IT\\Dev\\AgencyPrepayment\\Baton\\v' + gnmandolin_model_version + '\\batonutil\\',
                   ...
                   'fannie': {'model_version': baton_model_version,
                              'model_name': 'Baton_v' + baton_model_version,
                              'model_type': 'CONVENTIONAL',
                              'r_path': 'S:\\IT\\Dev\\AgencyPrepayment\\Baton\\v' + baton_model_version + '\\batonutil\\',
                   ...
                   'freddie': {'model_version': baton_model_version,
                               'model_name': 'Baton_v' + baton_model_version,
                               'model_type': 'CONVENTIONAL',
                               'r_path': 'S:\\IT\\Dev\\AgencyPrepayment\\Baton\\v' + baton_model_version + '\\batonutil\\',
                               ...
                               'vintage_list': [1950, 1989, 1991, 1992, 1993, 1994, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003,
                                2004, 2006, 2008, latest_year]},
                   'project_loan': {'model_version': glockenspiel_model_version,
                                    'model_name': 'Glockenspiel_v' + glockenspiel_model_version,
                                    'model_type': 'PROJECT_LOAN',
                                    'r_path': 'S:\\IT\\Dev\\AgencyPrepayment\\Glockenspiel\\v' + glockenspiel_model_version + '\\',
    ...
    global scale_path
    #scale_path = 'S:\\IT\\Production\\ScaleMQ\\release\\???\\'
    scale_path = 'S:\\IT\\Production\\ScaleMQ\\release\\5.21\\Scale_v2.02'
    ...
    #generate_data(full_update, factor_date, sample_only)

    # Step 2
    # Run Scale for each data file
    #generate_model_output(factor_date, use_r_engine)

    # Step 3
    # Load the model output into IQ
    load_model_data(factor_date, full_update)
