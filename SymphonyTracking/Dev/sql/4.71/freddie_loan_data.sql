...
SELECT
    ...
    CASE WHEN cl.marketTicker = 'FGU4' THEN 100 ELSE 0 END AS pct_FGU4,
    CASE WHEN cl.marketTicker = 'FGU7' THEN 100 ELSE 0 END AS pct_FGU7
INTO #Symphony_loans
...
WHERE 1=1
	AND sfl.version = (SELECT originationVersion_LL FROM scale.ModelVersionConfig WHERE appType = '9.98' AND agency = 'FHL' and tickerName = 'FGLMC')
    AND sli.version = (SELECT IncentiveVersion_LL FROM scale.ModelVersionConfig WHERE appType = '9.98' AND agency = 'FHL' and tickerName = 'FGLMC')
    AND slb.version = (SELECT burnoutVersion_LL FROM scale.ModelVersionConfig WHERE appType = '9.98' AND agency = 'FHL' and tickerName = 'FGLMC')
    @ASOF_WHERE@
	AND slh.asof > '2009-06-01'
    AND cl.marketTicker in ('FGCI','FGU4','FGU7','FGT4')
