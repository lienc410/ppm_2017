...
LEFT OUTER JOIN scale.GPL_Pool_ProjectedBalance sppb
    ...
    AND sppb.version = (SELECT ProjectedBalance_PL FROM scale.ModelVersionConfig WHERE appType = '3.10' AND agency = 'GPL' and tickerName = 'GPL')
LEFT JOIN #SurgeIndex sg
    ON sph.asOf = dateadd(month, +2, sg.asOf) -- same lag as MtgRate
WHERE 1=1
    AND spi.version = (SELECT IncentiveVersion_PL FROM scale.ModelVersionConfig WHERE appType = '3.10' AND agency = 'GPL' and tickerName = 'GPL')
    AND spb.version = (SELECT BurnoutVersion_PL FROM scale.ModelVersionConfig WHERE appType = '3.10' AND agency = 'GPL' and tickerName = 'GPL')
    @ASOF_WHERE@
