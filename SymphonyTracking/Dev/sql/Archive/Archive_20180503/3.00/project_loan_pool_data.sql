...
JOIN scale.GPL_PoolIncentive spi
    ON sph.loanNumber = spi.loanNumber
    AND sph.asOf = spi.asOf
JOIN scale.GPL_PoolBurnout spb
...
WHERE 1=1
    AND spi.version = (SELECT IncentiveVersion_PL FROM scale.ModelVersionConfig WHERE appType = '3.00' AND agency = 'GPL' and tickerName = 'GPL')
    AND spb.version = (SELECT BurnoutVersion_PL FROM scale.ModelVersionConfig WHERE appType = '3.00' AND agency = 'GPL' and tickerName = 'GPL')
    @ASOF_WHERE@
    AND sfo.schamBalance > 0.01
    AND sph.wala >= 0
    AND s.poolIsUnique = 1
    AND s.poolType in ('PL','PN','LM')
