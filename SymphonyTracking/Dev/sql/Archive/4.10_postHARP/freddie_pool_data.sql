...
WHERE 1=1
    AND spi.version = (SELECT IncentiveVersion_PL FROM scale.ModelVersionConfig WHERE appType = '4.00' AND agency = 'FHL' and tickerName = 'FGLMC')
    AND spb.version = (SELECT BurnoutVersion_PL FROM scale.ModelVersionConfig WHERE appType = '4.00' AND agency = 'FHL' and tickerName = 'FGLMC')
    @ASOF_WHERE@
    AND sf.schamBalance > 0.01
    AND sf.asOf >= '20170101' and sf.asOf<='20180301'
    AND sf.calcOrigMonth >= 200907 and sf.calcOrigMonth<=201803
    AND s.marketTicker = 'FGLMC'
;
COMMIT;
