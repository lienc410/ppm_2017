...
WHERE 1=1
    AND spi.version = (SELECT IncentiveVersion_PL FROM scale.ModelVersionConfig WHERE appType = '@DATABASE@' AND agency = 'FNM' and tickerName = 'FNCL')
    AND spb.version = (SELECT BurnoutVersion_PL FROM scale.ModelVersionConfig WHERE appType = '@DATABASE@' AND agency = 'FNM' and tickerName = 'FNCL')
    @ASOF_WHERE@
