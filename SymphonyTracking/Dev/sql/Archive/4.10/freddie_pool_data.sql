...
WHERE 1=1
    AND spi.version = (SELECT IncentiveVersion_PL FROM scale.ModelVersionConfig WHERE appType = '@DATABASE@' AND agency = 'FHL' and tickerName = 'FGLMC')
    AND spb.version = (SELECT BurnoutVersion_PL FROM scale.ModelVersionConfig WHERE appType = '@DATABASE@' AND agency = 'FHL' and tickerName = 'FGLMC')
    @ASOF_WHERE@
