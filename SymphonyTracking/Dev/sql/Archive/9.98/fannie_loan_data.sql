...
SELECT
	...
	CASE WHEN (clh.cltv=0 or slh.cltv=0.01) THEN 0.0
	ELSE 
		CASE WHEN clh.calcScham IS NOT NULL THEN ((clh.calcScham / clh.cltv) / (b.originalLoanAmount / sl.origLTV) - 1) * 100 
		ELSE ((clh.currentRPB / slh.cltv) / (b.originalLoanAmount / sl.origLTV) - 1) * 100 END 
	END as hpa_cum,
...
JOIN 
(
    SELECT loanSeqNum, asOf, currentRpb, loanAge, schamBalance, calcScham FROM fnm.PIV_Derived_LoanHist
    UNION ALL
    SELECT loanSeqNum, asOf, currentRpb, loanAge, schamBalance, calcScham FROM fnm.PIV_LoanHist WHERE asOf >= '20140101'
