...
SELECT
	...
	CASE WHEN clh.loanAge > 0 THEN 
			CASE WHEN clh.calcScham IS NOT NULL THEN (power((clh.calcScham / (case when clh.cltv<0 then slh.cltv else clh.cltv end)) / (cl.originalLoanAmount / sfl.origLTV), 12.0 / clh.loanAge) - 1.0) * 100 
				ELSE (power((clh.currentRPB / slh.cltv) / (cl.originalLoanAmount / sfl.origLTV), 12.0 / clh.loanAge) - 1.0) * 100 END
		ELSE 0.0 END as hpa_annual,
    CASE WHEN clh.calcScham IS NOT NULL THEN ((clh.calcScham / (case when clh.cltv<0 then slh.cltv else clh.cltv end)) / (cl.originalLoanAmount / sfl.origLTV) - 1) * 100 
         ELSE ((clh.currentRPB / slh.cltv) / (cl.originalLoanAmount / sfl.origLTV) - 1) * 100 END as hpa_cum,
	CASE WHEN cl.marketTicker not in ('FGU4','FGU7') and cl.loanPurposeType = 'PURCH' THEN 100.0 ELSE 0.0 END AS pct_purchase,
    ...
    CASE WHEN cl.marketTicker = 'FGU5' THEN 100 ELSE 0 END AS pct_FGU5,
    CASE WHEN cl.marketTicker = 'FGU8' THEN 100 ELSE 0 END AS pct_FGU8
INTO #Symphony_loans
...
WHERE 1=1
	AND sfl.version = (SELECT originationVersion_LL FROM scale.ModelVersionConfig WHERE appType = '4.70' AND agency = 'FHL' and tickerName = 'FGTW')
    AND sli.version = (SELECT IncentiveVersion_LL FROM scale.ModelVersionConfig WHERE appType = '4.70' AND agency = 'FHL' and tickerName = 'FGTW')
    AND slb.version = (SELECT burnoutVersion_LL FROM scale.ModelVersionConfig WHERE appType = '4.70' AND agency = 'FHL' and tickerName = 'FGTW')
    @ASOF_WHERE@
	AND slh.asof > '2009-06-01'
    AND cl.marketTicker in ('FGTW','FGU5','FGU8','FGT5')
