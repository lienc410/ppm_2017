...
--    CASE WHEN sli.conventional_refi_incentive_fixed_cost >= sli.fha_refi_incentive_fixed_cost THEN sli.conventional_refi_incentive_fixed_cost ELSE sli.fha_refi_incentive_fixed_cost END as refinance_incentive,    
    ...
    sl.origLTV as oltv,
    CASE WHEN cl.calcOrigMonth_LL < 200907 THEN 100 ELSE 0 END as pct_preHARP,
...
LEFT JOIN scale.servicerBuyoutIndex sbi
    ...
    AND sbi.version = '2.0'    
WHERE 1=1
    AND sl.version = (SELECT originationVersion_LL FROM scale.ModelVersionConfig WHERE appType = '4.60' AND agency = 'GNM' and tickerName = 'G2SF')
    AND sli.version = (SELECT IncentiveVersion_LL FROM scale.ModelVersionConfig WHERE appType = '9.99' AND agency = 'GNM' and tickerName = 'G2SF')
    AND slb.version = (SELECT BurnoutVersion_LL FROM scale.ModelVersionConfig WHERE appType = '9.99' AND agency = 'GNM' and tickerName = 'G2SF')
    AND slh.asOf >= '2009-07-01'
    AND slh.schamBalance > 0.01
    AND sl.loanType in ('VA')
--    AND sl.loanType in ('FHA','PIH', 'RHS')
--    AND slh.asOf >= '2018-01-01'
    @ASOF_WHERE@
;
COMMIT;
