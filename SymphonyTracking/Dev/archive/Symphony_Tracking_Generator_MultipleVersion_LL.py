...
def generate_loan_data_conv(run_ticker_list, full_update, factor_date, sample_only, model):
    ...
    def process(ticker):
        ...
        if sample_only: 
                sample_join     = ' JOIN scale.SampleLoanSeqNums si ON sfl.loanSeqNum = si.loanSeqNum '
...
def load_model_data_loan(run_ticker_list, factor_date, full_update, model):
    ...
    for run_ticker in run_ticker_list:
        ...
        full_update_SQLStmt = "delete " + tableName + "  where modelId = '" + model_version + "'  and issueId in (select issueId from (select issueId,agency from fnm.Sec UNION ALL select issueId,agency from fhl.Sec UNION ALL select issueId,agency from gnm.Sec ) t where agency = '"+agency+"')"
        full_update_SQLStmt_2 = "delete " + tableName + "  where modelId = '99'  and issueId in (select issueId from (select issueId,agency from fnm.Sec UNION ALL select issueId,agency from fhl.Sec UNION ALL select issueId,agency from gnm.Sec ) t where agency = '"+agency+"')"
...
def main(argv):
    ...
    scale_threads = 6 # adjust based on how much CPU you want to allocate to Scale (1 = lowest, 6 = highest)
    ...
    model_scale_path = {'4.00': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.35\\Scale_v2.02',
                   ...
                   '4.10': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.40\\Scale_v2.02',
                   ...
                   '9.97': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.40\\Scale_v2.02',
                           'gnmandolin_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.40\\Scale_v2.02',
                           'glockenspiel_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.40\\Scale_v2.02'},
                   '9.98': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.50_dev\\Scale_v2.02',
                           'gnmandolin_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.50_dev\\Scale_v2.02',
                           'glockenspiel_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.50_dev\\Scale_v2.02'}
                   }
    global model_dict # To store  executable for a model
    model_dict = {'4.00': {'baton_model_name': 'Baton_v4.00',
