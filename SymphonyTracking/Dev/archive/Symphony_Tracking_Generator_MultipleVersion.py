...
def generate_data(full_update, factor_date, sample_only, model):
    ...
    if 'ginnie' in running_ticker_list:
        ...
        for ticker in ginnie_list:
            generate_loan_data(ginnie_list, full_update, factor_date, sample_only, model)
        running_ticker_list.remove('ginnie')
    
    if len(running_ticker_list):
        generate_pool_data(running_ticker_list, full_update, factor_date, sample_only, model)
...
def generate_model_output(factor_date, use_r_engine, model):
    ...
    if 'ginnie' in running_ticker_list:
        ginnie_list = ['ginnie']
        for ticker in ginnie_list:
            generate_model_output_loan(ginnie_list, factor_date, use_r_engine, model)
        running_ticker_list.remove('ginnie')
    
    if len(running_ticker_list):
        generate_model_output_pool(running_ticker_list, factor_date, use_r_engine, model)
...
def load_model_data(factor_date, full_update, model):
    ...
    if 'ginnie' in running_ticker_list:
        ginnie_list = ['ginnie']
        for ticker in ginnie_list:
            load_model_data_loan(ginnie_list, factor_date, full_update, model)
        running_ticker_list.remove('ginnie')
    
    if len(running_ticker_list):
        load_model_data_pool(running_ticker_list, factor_date, full_update, model)
    
    print "Exiting load_model_data"

def generate_pool_data(run_ticker_list, full_update, factor_date, sample_only, model):
    ...
    def process(ticker):
        ...
        if sample_only: #remove later
                sample_join     = ' JOIN scale.SampleIssueIds si ON s.issueId = si.issueId '
...
def generate_loan_data(run_ticker_list, full_update, factor_date, sample_only, model):
    ...
    def process(ticker):
        ...
        #if not sample_only:
            if 1 == 0:
                print "Processing for Ticker For Sample(Sample for Scale tie-out):" + ticker + '_' + loan_type + " Started"
                extract_file = os.path.join(sql_path, extract_sql_file)
                extract_file_path = extract_path + '/' + 'PrepayLoanData_' + ticker +  '_' + loan_type + '_' + 'Scale_Tieout_sample.csv'
                vintage_where = ' AND 1=1 '
                loantype_where = " AND loanType = '%s' " % loan_type
                sample_join = ' JOIN report.Scale_V2_conv_samples_id si ON slh.issueId = si.issueId '
                print extract_file_path
                with open(extract_file, 'r') as inp:
                    file_contents = inp.read().replace("@EXTRACT_FILE_PATH@", extract_file_path)
                    file_contents = file_contents.replace("@VINTAGE_WHERE@", vintage_where)
                ...
                print "Processing for Ticker For Sample(Sample for Scale tie-out):" + ticker + '_' + loan_type + " Completed"
    ...
    print "Exiting generate_loan_data"

def generate_model_output_pool(run_ticker_list, factor_date, use_r_engine, model):
    ...
    print "Exiting generate_model_output_for_pools"


def generate_model_output_loan(run_ticker_list, factor_date, use_r_engine, model):
    ...
    print "Exiting generate_model_output_for_loans"
    
    
def load_model_data_pool(run_ticker_list, factor_date, full_update, model):
    print "Starting load_model_data_pool"
    # Constants
    ...
    for run_ticker in run_ticker_list:
        ...
        cursor.commit()
    print "Finish load_model_data_pool"
...
def load_model_data_loan(run_ticker_list, factor_date, full_update, model):
    ...
    for run_ticker in run_ticker_list:
        ...
        print "Cleaning Loan Up Old Data"
        cleanupSQLStmt = "delete " + tableName + "  where modelId = '" + model_version + "' or modelId is  NULL" if full_update else " delete staging_LoanPrepayTracking"
        print (cleanupSQLStmt)
        cursor.execute(cleanupSQLStmt)
        cursor.commit()
        ...
        symphony_tracking_path = 'S:/IT/Production/Scripts/python/Scale/SymphonyTracking/' + environment + '/'
        sql_path = os.path.join(symphony_tracking_path, 'sql/'+ model)
        load_file = os.path.join(sql_path, "loadLoanPrepay_old.sql")
        with open(load_file, 'r') as sqlFile:
            loadSQLStmt = sqlFile.read()
        loadSQLStmt = loadSQLStmt.replace("@TABLE@", tableName)
        print loadSQLStmt
        ...
        aggregation_file = os.path.join(sql_path, "ginnie_loan_aggregation.sql")
        ...
        # In case of factor_update insert data from staging to main table
        update_file = os.path.join(sql_path, "updateLoanPrepay_old.sql")
        if not (full_update):
            with open(update_file, 'r') as sqlFile:
               updateSQLStmt = sqlFile.read()
            print (updateSQLStmt)
...
def main(argv):
    ...
    scale_threads = 4 # adjust based on how much CPU you want to allocate to Scale (1 = lowest, 6 = highest)
    ...
    model_scale_path = {'3.00': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.32\\Scale_v2.02',
                           'gnmandolin_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.32\\Scale_v2.02',
                           'glockenspiel_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.32\\Scale_v2.02'},
                   '3.10': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.37\\Scale_v2.02',
                           'gnmandolin_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.37\\Scale_v2.02',
                           'glockenspiel_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.37\\Scale_v2.02'}, 
                   '4.00': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.35\\Scale_v2.02',
                           'gnmandolin_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.35\\Scale_v2.02',
                           'glockenspiel_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.36_dev\\Scale_v2.02'},
                   '4.10': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.43\\Scale_v2.02',
                           'gnmandolin_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.43v\\Scale_v2.02',
                           'glockenspiel_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.43\\Scale_v2.02'},
                   '4.20': {'baton_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.43\\Scale_v2.02',
                           'gnmandolin_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.43\\Scale_v2.02',
                           'glockenspiel_model_scale_path': 'S:\\IT\\Production\\ScaleMQ\\release\\5.43\\Scale_v2.02'}
                   }
    global model_dict # To store  executable for a model
    model_dict = {'3.00': {'baton_model_name': 'Baton_v3.00',
                           'baton_model_version': '3.00',
                           'gnmandolin_model_name': 'Gnmandolin_v3.00',
                           'gnmandolin_model_version': '3.00',
                           'glockenspiel_model_name': 'Glockenspiel_v1.71',
                           'glockenspiel_model_version': '1.71'},
                   '3.10': {'baton_model_name': 'Baton_v3.10',
                           'baton_model_version': '3.10',
                           'gnmandolin_model_name': 'Gnmandolin_v3.10',
                           'gnmandolin_model_version': '3.10',
                           'glockenspiel_model_name': 'Glockenspiel_v1.71',
                           'glockenspiel_model_version': '1.71'},
                   '4.00': {'baton_model_name': 'Baton_v4.00',
                           'baton_model_version': '4.00',
                           'gnmandolin_model_name': 'Gnmandolin_v4.00',
                           'gnmandolin_model_version': '4.00',
                           'glockenspiel_model_name': 'Glockenspiel_v2.00',
                           'glockenspiel_model_version': '2.00'},
                   '4.10': {'baton_model_name': 'Baton_v4.10',
                           'baton_model_version': '4.10',
                           'gnmandolin_model_name': 'Gnmandolin_v4.00',
                           'gnmandolin_model_version': '4.00',
                           'glockenspiel_model_name': 'Glockenspiel_v2.00',
                           'glockenspiel_model_version': '2.00'},
                   '4.20': {'baton_model_name': 'Baton_v4.20',
                           'baton_model_version': '4.20',
                           'gnmandolin_model_name': 'Gnmandolin_v4.00',
                           'gnmandolin_model_version': '4.00',
                           'glockenspiel_model_name': 'Glockenspiel_v2.00',
                           'glockenspiel_model_version': '2.00'}
    ...
    ticker_dict = {'ginnie': {'model':'gnmandolin_model',
	                          'model_type': 'GINNIE',
	                          'r_path': 'S:\\IT\\Dev\\AgencyPrepayment\\Baton\\v' + '\\batonutil\\',
                              ...
                              'vintage_list': [2002, 2003, 2006, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, latest_year]},
                   'fannie': {'model':'baton_model',
                              ...
                              'media_sql': 'media_effect_data.sql',
                              'credit_sql': 'credit_availability_data.sql',
                              'data_sql': 'fannie_pool_data.sql',
                              'extract_sql': 'conventional_data_extract.sql',
                              'vintage_list': [1950, 1989, 1991, 1992, 1993, 1994, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003,
                               2004, 2006, 2008, latest_year]},
                   'freddie': {'model':'baton_model',
	                          ...
	                          'r_path': 'S:\\IT\\Dev\\AgencyPrepayment\\Baton\\v'  + '\\batonutil\\',
                               ...
                               'media_sql': 'media_effect_data.sql',
                               'credit_sql': 'credit_availability_data.sql',
                               'data_sql': 'freddie_pool_data.sql',
                               'extract_sql': 'conventional_data_extract.sql',
                               'vintage_list': [1950, 1989, 1991, 1992, 1993, 1994, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003,
                                2004, 2006, 2008, latest_year]},
                   'project_loan': {'model':'glockenspiel_model',
                                    ...
                                    'data_sql': 'project_loan_pool_data_Intex_pool.sql',
                                    'extract_sql': 'project_loan_data_extract.sql',
