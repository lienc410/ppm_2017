...
ppdata$lomedia_elbow_wacls     <- ppdata$wacls

elbow_list_refinance  <- c("lomedia_elbow_wacls")

elbow.submdl <-  names(refi.mdl)[match(elbow_list_refinance, names(refi.mdl))]
elbow_mult.df <- data.frame(matrix(nrow = nrow(ppdata), ncol = length(elbow.submdl)))
names(elbow_mult.df) <- elbow.submdl
                
for (p in elbow.submdl) {
    if(!(p %in% names(ppdata))) {
        elbow_mult.df[, p] <- 1.0
        cat("ATTENTION!Missing Parameter for elbow shift in INPUT!", " missing: ", p, "\n")
    }
    else {
        idx <- match(p, names(refi.mdl))
        a <- unlist(refi.mdl[idx][[1]][1])  # Otherwise R complains about not being able to coerce a list into a double
...
ppdata$lomedia_incentive_adj  <- elbow_mult.df$lomedia_elbow_wacls

#cat("adjustment: ", ppdata$lomedia_incentive_adj, "\n")

ppdata$incentive.store    <- ppdata$incentive
ppdata$lomedia_incentive_burnout0  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
ppdata$lomedia_incentive_burnout500  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
ppdata$lomedia_incentive_burnout1500  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
ppdata$lomedia_incentive_burnout2500  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
ppdata$lomedia_incentive_burnout3500  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
ppdata$lomedia_incentive_burnout4500  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
ppdata$lomedia_incentive_burnout5500  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
ppdata$lomedia_incentive_burnout7000  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
ppdata$lomedia_incentive_burnout8000  <- ppdata$refinance_incentive + ppdata$lomedia_incentive_adj
...
ppdata$curt       <- PredictSubmodel_4_50(coll=coll, submodel="curt", submdl.spl=curt.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$pred
...
ppdata$incentive  <- ppdata$incentive.store
ppdata$cout       <- PredictSubmodel_4_50(coll=coll, submodel="cout", submdl.spl=cout.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$pred * ppdata$bizDatCntAdj
cat("Calculate Cashout: Done", "\n")

# Use unadjusted incentive for turnover predictions
cat("Calculate Turnover: Start", "\n")
ppdata$incentive  <- ppdata$incentive.store  
ppdata$turn       <- PredictSubmodel_4_50(coll=coll, submodel="turn", submdl.spl=turn.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$pred * ppdata$bizDatCntAdj
...
ppdata$refi       <- PredictSubmodel_4_50(coll=coll, submodel="refi", submdl.spl=refi.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$pred * ppdata$bizDatCntAdj
...
# turn_test       <- PredictSubmodel_4_50(coll=coll, submodel="turn", submdl.spl=turn.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$mult
# refi_test       <- PredictSubmodel_4_50(coll=coll, submodel="refi", submdl.spl=refi.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$mult
