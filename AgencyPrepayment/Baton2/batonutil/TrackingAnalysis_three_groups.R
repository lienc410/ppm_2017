...
# step.df         <- Bucket.Step(step.by = 'wala',      step = 5,  from = 0,   to = 50,   overwrite = TRUE,  numeric = TRUE, spline = TRUE)
# step.df         <- Bucket.Step(step.by = 'incentive', step = 25, from = -50, to = -25,  overwrite = TRUE,  numeric = TRUE, spline = FALSE)
# step.df         <- Bucket.Step(step.by = 'wacls',     step = 50, from = 0,    to = 300, overwrite = TRUE,  numeric = TRUE)
step.df         <- Bucket.Step(step.by = 'burnout',   step = 2000, from = 2000, to = 10000, overwrite = TRUE,  numeric = TRUE, spline = TRUE)
...
hpa.grp.uniq <- unique(subdata$hpa2yrBucket)
inc.grp.uniq <- unique(subdata$incentiveBucket)
subdata1 <- subdata

plot.time.stamp   <- format(Sys.time(),'%m_%d_%H_%M')
main.polder.name  <- paste("../output/Burnout_", plot.time.stamp, sep='')
dir.create(main.polder.name)

for(inc.i in 1: length(inc.grp.uniq)){
  subdata2 <- subset(subdata1, incentiveBucket == inc.grp.uniq[inc.i])
  
for(hpa.i in 1:length(hpa.grp.uniq)){

subdata <- subset(subdata2, hpa2yrBucket == hpa.grp.uniq[hpa.i])

plot.folder.name  <- paste("../output/WALA_", plot.time.stamp, "/ict_", inc.grp.uniq[inc.i],"_hpa_", hpa.grp.uniq[hpa.i], sep='')
...
for (row in 2:nrow(step.df)) {
  ...
  agg.data.smm       <- ddply(subdata, .(grp = subdata[[group.by.bucket]], x = subdata[[plot.by.bucket]]), summarise, 
                              ...
                              model.smm      = 100 * weighted.mean(modelSMM_v4.10, bal),
                              ratio          = weighted.mean(smm, bal) / weighted.mean(modelSMM_v4.10, bal),
                              track.err      = 100 * weighted.mean(modelSMM_v4.10 - smm, bal),
                              curt.smm       = 100 * weighted.mean(curt_v4.10, bal),
                              dflt.smm       = 100 * weighted.mean(dfltall_v4.10, bal),
                              cout.smm       = 100 * weighted.mean(cout_v4.10, bal),
                              turn.smm       = 100 * weighted.mean(turn_v4.10, bal),
                              refi.smm       = 100 * weighted.mean(refi_v4.10, bal),
                              ...
                              hpa2yr	       =	weighted.mean(hpa2yr, bal),
                              media_effect	 =	weighted.mean(media_effect, bal),
                              sato	         =	weighted.mean(sato, bal),
                              # turn_cai_mult	= weighted.mean(	turn_cai_v4.10	,bal),
                              # turn_hpa2yr_mult	= weighted.mean(	turn_hpa2yr_v4.10	,bal),
                              # turn_wala_mult	= weighted.mean(	turn_wala_v4.10	,bal),
                              # turn_incentive_mult	= weighted.mean(	turn_incentive_v4.10	,bal),
                              # turn_cltv_mult	= weighted.mean(	turn_cltv_v4.10	,bal),
                              # turn_wacls_mult	= weighted.mean(	turn_wacls_v4.10	,bal),
                              # turn_fico_mult	= weighted.mean(	turn_fico_v4.10	,bal),
                              # turn_pct_REFI_mult	= weighted.mean(	turn_pct_REFI_v4.10	,bal),
                              # turn_tax_mult	= weighted.mean(	turn_tax_v4.10	,bal),
                              # turn_sato_mult	= weighted.mean(	turn_sato_v4.10	,bal),
                              # turn_pct_HARPed_mult	= weighted.mean(	turn_pct_HARPed_v4.10	,bal),
                              # turn_HARP_eligible_mult	= weighted.mean(	turn_HARP_eligible_v4.10	,bal),
  ...
  if(plot.spline){
    ...
    output.file <- paste(plot.folder.name, "/turn", "_", group.by, "_spline.csv",  sep = "")
