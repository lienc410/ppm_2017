...
PredictSubmodel_4_80_interpolation <- function(coll, submodel = NA, submdl.spl, data, seasadj = FALSE, version = "") {
    ...
    if (!any(is.na(submdl.spl))) { 
        ...
        removal_list_turnover_conv   <-  c("baselineConstant", "adjustmentFactor", "seasonalityMultipliers", "hpa_interp", "hpa_interp_U5", "hpa_interp_U8", "wala", "wala_U5", "wala_U8")
...
cat("refi mdl step9: results ", difftime(end_time9, start_time9, units='mins'), "\n")                            
              ...
              wala_submdl_U5     <- submdl.spl$wala_U5$functions
              wala_submdl_U8     <- submdl.spl$wala_U8$functions
            
              names(wala_submdl)         <- c("lohpa_wala","hihpa_wala")
              names(wala_submdl_U5)      <- c("lohpa_wala_U5","hihpa_wala_U5")
              names(wala_submdl_U8)      <- c("lohpa_wala_U8","hihpa_wala_U8")
           
              wala_list_turnover  <- c("lohpa_wala","hihpa_wala","lohpa_wala_U5","lohpa_wala_U8","hihpa_wala_U5","hihpa_wala_U8")
              ...
              names(wala.df) <- wala_list_turnover
                ...
                for (p in names(wala_submdl_U5)) {
                    if(!(p %in% names(ppdata))) {
                        wala.df[, p] <- 1.0
                        cat("ATTENTION!Missing wala_U5 in INPUT!", " missing: ", p, "\n")
                    }
                    else {
                        idx <- match(p, names(wala_submdl_U5))
                        a <- unlist(wala_submdl_U5[idx][[1]][1]) 
                        b <- unlist(wala_submdl_U5[idx][[1]][2])
                        x <- ppdata[, p]
                        wala.df[, p] <- BuildSpline(a = a, b = b, x = x)$y
                    }
                }

                for (p in names(wala_submdl_U8)) {
                    if(!(p %in% names(ppdata))) {
                        wala.df[, p] <- 1.0
                        cat("ATTENTION!Missing wala_U8 in INPUT!", " missing: ", p, "\n")
                    }
                    else {
                        idx <- match(p, names(wala_submdl_U8))
                        a <- unlist(wala_submdl_U8[idx][[1]][1]) 
                        b <- unlist(wala_submdl_U8[idx][[1]][2])
                        x <- ppdata[, p]
                        wala.df[, p] <- BuildSpline(a = a, b = b, x = x)$y
                    }
                }
              ...
              wala.df$wala_U5   <- (1.0 - ppdata$hpa_interpolation_U5) * wala.df$lohpa_wala_U5 + ppdata$hpa_interpolation_U5 * wala.df$hihpa_wala_U5
              wala.df$wala_U8   <- (1.0 - ppdata$hpa_interpolation_U8) * wala.df$lohpa_wala_U8 + ppdata$hpa_interpolation_U8 * wala.df$hihpa_wala_U8
              
              wala.df$wala_mult <- (ppdata$pct_FGU5 * wala.df$wala_U5 + ppdata$pct_FGU8 * wala.df$wala_U8 + (100 - ppdata$pct_FGU5 - ppdata$pct_FGU8) * wala.df$wala) / 100
