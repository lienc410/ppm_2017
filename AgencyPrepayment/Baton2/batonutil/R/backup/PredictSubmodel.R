...
#'  }
#'  
PredictSubmodel <- function(coll, submodel = NA, submdl.spl, data, seasadj = FALSE, version = "") {
    ...
    if (!any(is.na(submdl.spl))) {
        ...
        for (p in param.submdl) {
            ...
            # if ((p == "burnout") & (submodel == "refi")) {
            #     mult.df[, p] <- CalculateNewBurnout(F=mult.df[, p], L=data[, "wacls"])
            # }
            # if ((p == "eqlod") & (submodel == "turn")) {
            #   mult.df[, p] <- CalculateEQLODMultiplier(origMult=mult.df[, p], cltv=data[, "cltv"])
            # }
        }
        ...
        if (submodel == "refi") {
            if (coll == "gnma30") {
              submdl.pred <- submdl.pred * mult.df[, "cai"] * mult.df[, "hpa2yr"] * mult.df[, "burnout"] * mult.df[, "wacls"] * 
                mult.df[, "refi_elig_pct"] * mult.df[, "pct_tpo"] * mult.df[, "HARP_eligible"]  * mult.df[, "wala"] * mult.df[, "pct_second_lien"] * mult.df[, "fico"] *
                (mult.df[, "purch_oltv"] * data[, "pct_purchase"] + mult.df[, "refi_HARP_oltv"] * data[, "pct_HARPed"] + mult.df[, "refi_NonHARP_oltv"] * (100 - data[, "pct_purchase"] - data[, "pct_HARPed"])) *
                (mult.df[, "lomedia_incentive"] * (1 - mult.df[, "media_effect"]) + mult.df[, "himedia_incentive"] * mult.df[, "media_effect"]) / 100
            }
            else {
              submdl.pred <- submdl.pred * mult.df[, "cai"] * mult.df[, "hpa2yr"] * mult.df[, "burnout"] * mult.df[, "wacls"] * 
            ...
            submdl.pred <- submdl.pred * mult.df[, "cai"] * mult.df[, "hpa_annual"] * mult.df[, "incentive"] * 
              ...
              (mult.df[, "lohpa_wala"] * (1 - mult.df[, "pct_hihpa_wala"]) + mult.df[, "hihpa_wala"] * mult.df[, "pct_hihpa_wala"]) / 100
        }
        else if ((submodel == "turn") & (coll == "gnma30")) {
              submdl.pred <- submdl.pred * mult.df[, "cai"] * mult.df[, "hpa_annual"] * mult.df[, "incentive"] * 
              mult.df[, "cltv"] * mult.df[, "acls"] * mult.df[, "fico"] *mult.df[, "tax"] * mult.df[, "HARP_eligible"] * mult.df[, "pct_second_lien"] *
              (mult.df[, "purch_oltv"] * data[, "pct_purchase"] + mult.df[, "refi_HARP_oltv"] * data[, "pct_HARPed"] + mult.df[, "refi_NonHARP_oltv"] * (100 - data[, "pct_purchase"] - data[, "pct_HARPed"])) *
              (mult.df[, "fico_acls_interact_2_fico"] * mult.df[, "fico_acls_interact_1_acls"] + 1 *(1 - mult.df[, "fico_acls_interact_2_fico"])) *
              (mult.df[, "lohpa_wala"] * (1 - mult.df[, "pct_hihpa_wala"]) + mult.df[, "hihpa_wala"] * mult.df[, "pct_hihpa_wala"]) / 100
        }
