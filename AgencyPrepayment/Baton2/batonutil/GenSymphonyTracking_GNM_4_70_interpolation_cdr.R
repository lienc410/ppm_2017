...
#smm.df.csv    <- data.frame(paste(ppdata$loanseqnum, ppdata$asOf, ppdata$dfltall, ppdata$dfltTransit, ppdata$dfltBuyout, "",sep = "|"))
#fwrite(smm.df.csv, data.output, row.names = F, col.names = F, qmethod = "escape")

# dflt test
smm.df.csv        <- data.frame(cbind(ppdata$loanseqnum, ppdata$asOf, ppdata$bal, ppdata$buyoutIncentive))
names(smm.df.csv) <- c("loanSeqNum", "asOf", "bal", "buyuoutIncentive")
