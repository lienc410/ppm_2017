...
names(ppdata)   <- c("loanseqnum","asOf","monthBucket","bal","pct_owner","pct_2nd","pct_inv"
                     ...
                     ,"hpa2yr","cai","media_effect","day_count","wam","hpa_annual","hpa_cum","acls","pct_purchase","pct_second_lien", "refinance_incentive","oltv","pct_preHARP","pct_NY","pct_FGU5","pct_FGU8","NACol")
...
pre_list_turnover  <- c("hpa_interp","hpa_interp_U5","hpa_interp_U8") 
...
ppdata$hpa_interp_U5    <- ppdata$hpa_annual
ppdata$hpa_interp_U8    <- ppdata$hpa_annual
...
ppdata$hpa_interpolation_U5      <-   preparation_tover.df$hpa_interp_U5
ppdata$hpa_interpolation_U8      <-   preparation_tover.df$hpa_interp_U8
...
ppdata$lohpa_wala_U5      <- ppdata$wala
ppdata$lohpa_wala_U8      <- ppdata$wala

ppdata$hihpa_wala         <- ppdata$wala
ppdata$hihpa_wala_U5      <- ppdata$wala
ppdata$hihpa_wala_U8      <- ppdata$wala
...
ppdata$curt       <- PredictSubmodel_4_80_interpolation(coll=coll, submodel="curt", submdl.spl=curt.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$pred
...
ppdata$incentive  <- ppdata$incentive.store
ppdata$cout       <- PredictSubmodel_4_80_interpolation(coll=coll, submodel="cout", submdl.spl=cout.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$pred * ppdata$bizDatCntAdj
cat("Calculate Cashout: Done", "\n")

# Use unadjusted incentive for turnover predictions
cat("Calculate Turnover: Start", "\n")
ppdata$incentive  <- ppdata$incentive.store  
ppdata$turn       <- PredictSubmodel_4_80_interpolation(coll=coll, submodel="turn", submdl.spl=turn.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$pred * ppdata$bizDatCntAdj
...
ppdata$refi       <- PredictSubmodel_4_80_interpolation(coll=coll, submodel="refi", submdl.spl=refi.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$pred * ppdata$bizDatCntAdj
...
# refi_test       <- PredictSubmodel_4_80_interpolation(coll=coll, submodel="refi", submdl.spl=refi.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$mult
# turn_test       <- PredictSubmodel_4_80_interpolation(coll=coll, submodel="turn", submdl.spl=turn.mdl, data=ppdata, seasadj=TRUE, version=kNewModelVersion)$mult
