...
where loanSeqNum in (Select loanSeqNum from scale.fhl_missing_hist);
commit;
...
update #fhl_missing_data set version=(select incentiveVersion from #tmp_version);
commit;
...
UPDATE #LoanIncentive inc SET conv_incentive = CASE WHEN inc.marketTicker = 'FGTW' THEN origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 3.5) - pmi
                                                    ELSE origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 4.0) - pmi END;
delete from #LoanIncentive where conv_incentive is NULL;

UPDATE #LoanIncentive inc SET conv_incentive_fc = CASE WHEN inc.marketTicker = 'FGTW' THEN origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 3.5) - pmi - fixed_cost
