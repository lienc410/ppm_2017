...
-- Select Tickers Needed to be Back-Filled
...
--Find out the first asof saved in scale.fnm_loanhist
drop table if exists #fnm_first_asof;
...
drop table if exists #fnm_missing_hist_list;
select a.loanSeqNum
into #fnm_missing_hist_list
from #fnm_first_asof a
join scale.fnm_loan_dev l on a.loanseqnum=l.loanseqnum
where l.version in (select originationVersion from #tmp_version) 
and a.first_asof>l.originationDate;
commit;

--Remove the loans already available in the missing hist table. Only for new month.
IF (SELECT asOf FROM #tmp_asOf) = (SELECT asOf_last FROM #tmp_asOf)
    BEGIN
        Delete from #FNM_missing_hist_list
        where loanSeqNum in (Select loanSeqNum from scale.fnm_missing_hist mh inner join #tmp_version t on mh.version = t.incentiveVersion)
    END
;
COMMIT;
...
--Create temp table to store data to be added to scale.fnm_missing_hist later
drop table if exists #fnm_missing_data;
select
mhl.loanseqnum,
t.asof,
...
into #fnm_missing_data
from #fnm_missing_hist_list mhl
join #fnm_first_asof fa on mhl.loanseqnum=fa.loanseqnum
join scale.fnm_loan_dev l on mhl.loanseqnum=l.loanseqnum 
join #unique_dates t  on t.asOf >= l.originationDate and t.asOf < fa.first_asOf  
where l.version in (select originationVersion from #tmp_version);
commit;

update #fnm_missing_data set version=(select distinct incentiveVersion from #tmp_version);
commit;
...
cast (null as numeric(14,2)) as schamBalance
into #balance
from #fnm_missing_data mh
join scale.fnm_loan_dev l on mh.loanseqnum=l.loanseqnum
join fnm.PIV_loan cl on mh.loanseqnum = cl.loanseqnum
where l.version in (select originationVersion from #tmp_version);
commit;
...
Update #fnm_missing_data mh set mh.currentBalance=b.currentBalance, mh.schamBalance=b.schamBalance
...
from hpi.PIV_HomePriceIndexMonthly hpi 
...
UPDATE #fnm_missing_data mh set mh.cltv=c.cltv
...
and loanseqnum in (select loanseqnum from #MICancel);
COMMIT;
...
UPDATE #fnm_missing_data mh set mh.MICancelFlag_unlagged=m.MICancelFlag_unlagged,mh.MICancelFlag_lagged=m.MICancelFlag_lagged
FROM #MICancel m WHERE mh.loanseqnum=m.loanseqnum and mh.asof=m.asof;
COMMIT;

UPDATE #fnm_missing_data mh set mh.MICancelFlag_unlagged = 'N'
WHERE mh.MICancelFlag_unlagged is NULL;
COMMIT;

UPDATE #fnm_missing_data mh set mh.MICancelFlag_lagged = 'N'
WHERE mh.MICancelFlag_lagged is NULL;
COMMIT;

-- MI Cancellation based on model
-- Original PMI Cancellation, if flag='Y' then cancel
UPDATE #fnm_missing_data i SET i.origPMI=0
WHERE MICancelFlag_lagged='Y';
COMMIT;

-- Original PMI haircut
UPDATE #fnm_missing_data i SET i.origPMI=0.85*i.origPMI
...
FROM #fha_eligibility e, fnm.PIV_Loan cl
WHERE e.loanSeqNum = cl.loanSeqNum
AND cl.occType in ('2ND', 'INV')
;
COMMIT;

UPDATE #fnm_missing_data mh set mh.HARP_Eligible=h.HARP_Eligible
FROM #harp_eligibility h where mh.loanseqnum=h.loanseqnum and mh.asof=h.asof;
COMMIT;

UPDATE #fnm_missing_data mh set mh.conventional_Eligible=h.conventional_Eligible
FROM #conventional_eligibility h where mh.loanseqnum=h.loanseqnum and mh.asof=h.asof;
COMMIT;

UPDATE #fnm_missing_data mh set mh.fha_eligible=h.fha_eligible
...
UPDATE #LoanIncentive inc SET inc.llpa = 0 WHERE inc.marketTicker = 'FNCI';
COMMIT;

-- Update Conventional Refinance Incentive
UPDATE #LoanIncentive inc SET conv_incentive = CASE WHEN inc.marketTicker = 'FNCT' THEN origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 3.5) - pmi - mortgage_tax
...
UPDATE #LoanIncentive inc SET conv_incentive_fc = CASE WHEN inc.marketTicker = 'FNCT' THEN origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 3.5) - pmi - fixed_cost - mortgage_tax
...
UPDATE #LoanIncentive inc SET fha_incentive = NULL WHERE inc.marketTicker in ('FNCI', 'FNCT');
UPDATE #LoanIncentive inc SET fha_incentive_fc = NULL WHERE inc.marketTicker in ('FNCI', 'FNCT');
...
UPDATE #fnm_missing_data mh set 
...
Delete from scale.fnm_missing_hist
from scale.fnm_missing_hist mh, #fnm_missing_data md
Where mh.loanSeqNum = md.loanSeqNum
and mh.version = md.version;
COMMIT;

--Transfer the data to the missing hist table
Insert into scale.fnm_missing_hist
(
loanSeqNum,
asOf,
currentBalance,
...
SELECT @cnt = count(*)
FROM scale.FNM_LoanIncentive i
Inner Join scale.FNM_missing_hist m
