...
VALUES (getdate(),'FHL_Pool_Step 5_Create_Pool_Incentive_Table','START');
COMMIT;
...
JOIN #tmp_version tpv
    ON pi.marketTicker = tpv.tickerName
...
UPDATE #FHL_PoolBasedIncentive inc SET inc.llpa = 0 WHERE inc.marketTicker = 'FGCI';
COMMIT;
...
UPDATE #FHL_PoolBasedIncentive inc SET
    conventional_refi_incentive = 100.0 * (CASE WHEN inc.marketTicker = 'FGTW' THEN wac + (pmi_begin / 100.0) - MtgRate - (llpa / 350.0) - (pmi / 100.0)
...
UPDATE #FHL_PoolBasedIncentive inc SET fha_refi_incentive = Null WHERE inc.marketTicker in ('FGCI', 'FGTW');
UPDATE #FHL_PoolBasedIncentive SET
...
UPDATE #FHL_PoolBasedIncentive inc SET
    refi_incentive_eligible = 0
WHERE refi_incentive_eligible IS NULL;
COMMIT;
...
--select * from #FHL_PoolBasedIncentive where refi_incentive_eligible IS NOT NULL
-- Tests
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Pools with WRONG VALUE for conventional_refi_incentive: %1!', @cnt
            RETURN
        END
...
--       if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'Number of FHL Pools with WRONG VALUE for fha_refi_incentive: %1!', @cnt
--            RETURN
--        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Pools with WRONG VALUE for refi_incentive: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Pools with WRONG VALUE for refi_incentive_eligible: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Pools with WRONG VALUE for pmi_begin: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Pools with WRONG VALUE for llpa: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Pools with WRONG VALUE for pmi: %1!', @cnt
            RETURN
        END
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'Number of FHL Pools with WRONG VALUE for mip: %1!', @cnt
--            RETURN
--        END
----------------------------------------------------------------------------------------------
...
SELECT
    issueId,
    asOf,
    tpv.PLincentiveVersion as version,
...
INSERT INTO scale.JobStatus (EndTime, JobName, JobStatus, #LoansBeforeProcess, #LoansUpdated, #LoansAfterProcess)
    SELECT
     getdate(),
    'FHL_Pool_Step 5_Create_Pool_Incentive_Table',
    'SUCCESS', 
    @count_previous,
    @count_current,
