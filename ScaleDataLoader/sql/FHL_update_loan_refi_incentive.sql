...
-- Save Loan Data
DROP TABLE IF EXISTS #FHL_LoanIncentive1;
SELECT
    cl.loanSeqNum ,
    cl.marketTicker,
	cl.state,
...
INTO #FHL_LoanIncentive1
FROM scale.FHL_RefreshLoans srl
JOIN fhl.PIV_Loan cl
    ON cl.loanSeqNum = srl.loanSeqNum
JOIN #ticker tk
...
DELETE FROM #FHL_LoanIncentive
FROM #FHL_LoanIncentive li
JOIN #FHL_LoanIncentive1 li1
ON li.loanSeqNum = li1.loanSeqNum
AND li.asOf = li1.asOf;

--Inserting entire data for refresh loans.
INSERT INTO #FHL_LoanIncentive
SELECT * FROM #FHL_LoanIncentive1;
...
FROM #FHL_LoanIncentive inc,report.conforming_loansize_limit cll
...
UPDATE #FHL_LoanIncentive inc 
SET conv_incentive_cltv50 = CASE WHEN inc.marketTicker = 'FGTW' THEN origNoteRate + (origPMI / 100.0) - MtgRate - (llpa_cltv50 / 3.5) - pmi_cltv50 - mortgage_tax
...
UPDATE #FHL_LoanIncentive inc 
SET conv_incentive_fc_cltv50 = CASE WHEN inc.marketTicker = 'FGTW' THEN origNoteRate + (origPMI / 100.0) - MtgRate - (llpa_cltv50 / 3.5) - pmi_cltv50 - fixed_cost - mortgage_tax
...
UPDATE #FHL_LoanIncentive inc SET fha_incentive = origNoteRate + (origPMI / 100.0) - MtgRate_GNMA - mip - mortgage_tax;
UPDATE #FHL_LoanIncentive inc SET fha_incentive = 0 WHERE fha_incentive is NULL;

UPDATE #FHL_LoanIncentive inc SET fha_incentive_fc = origNoteRate + (origPMI / 100.0) - MtgRate_GNMA - mip - fixed_cost - mortgage_tax;
UPDATE #FHL_LoanIncentive inc SET fha_incentive_fc = 0 WHERE fha_incentive_fc is NULL;

-- Update FHA Refinance Incentive CLTV50
UPDATE #FHL_LoanIncentive inc SET fha_incentive_cltv50 = origNoteRate + (origPMI / 100.0) - MtgRate_GNMA - mip_cltv50 - mortgage_tax;
UPDATE #FHL_LoanIncentive inc SET fha_incentive_cltv50 = 0 WHERE fha_incentive_cltv50 is NULL;

UPDATE #FHL_LoanIncentive inc SET fha_incentive_fc_cltv50 = origNoteRate + (origPMI / 100.0) - MtgRate_GNMA - mip_cltv50 - fixed_cost - mortgage_tax;
