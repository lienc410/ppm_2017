...
VALUES (getdate(),'GNM_Loan_Step 4_Create_Loan_Eligibility_Table','START');
COMMIT;
...
--SELECT CAST ('2018-02-01' AS DATE) asOf, CAST ('2018-03-01' AS DATE) asOf_last INTO #tmp_asOf
;
...
--            AND cl.numUnits = llpa_unit.numUnits
...
-- Update PMI
UPDATE #GNM_conventional_eligibility ce
...
FROM report.PIV_PMI_Base_Raw pmi_base
...
FROM report.PIV_LLPA_Units_Raw llpa_unit
    WHERE ce.asOf >= llpa_unit.startDate AND ce.asOf < llpa_unit.endDate
            AND ce.numberOfUnits = llpa_unit.numUnits
...
SET ConvCost = base_PMI +occ_PMI + size_PMI + base_LLPA + ltv_LLPA + admc_LLPA + occ_LLPA + unit_LLPA + size_LLPA,
       ConvCostValidility = CASE WHEN ce.origFICO IS NULL OR ce.cltv IS NULL OR ce.numberOfUnits IS NULL OR ce.currentBalance IS NULL THEN 'N' ELSE 'Y' END;
COMMIT;
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)  
        BEGIN
            RAISERROR 99999 'Number of GNM Loans with NULL Value for Conventional Refinance Eligibility: %1!', @cnt
            RETURN
        END 
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)  
        BEGIN
            RAISERROR 99999 'Number of GNM Loans with INVALID Value for Conventional Refinance Eligibility: %1!', @cnt
            RETURN
        END 
----------------------------------------------------------------------------------------------
-- Check no duplicates
...
--        SELECT
 ...
 --       if (@cnt > 0) 
--        BEGIN
 --           RAISERROR 99999 'Number of Duplicates for GNM loans in Loan Eligibility Table: %1!', @cnt
 --           RETURN
 --       END 
...
--	 FROM scale.GNM_LoanEligibility
...
INSERT INTO scale.JobStatus (EndTime, JobName, JobStatus, #LoansBeforeProcess, #LoansUpdated, #LoansAfterProcess)
    SELECT
     getdate(),
    'GNM_Loan_Step 4_Create_Loan_Eligibility_Table', 
    'SUCCESS', 
    @count_previous,
    @count_current,
