...
VALUES (getdate(),'FNM_Pool_Step 5_Create_Pool_Incentive_Table','START');
COMMIT;
...
SELECT mvc.tickerName, mvc.appType, mvc.incentiveVersion_LL as LLincentiveVersion, mvc.incentiveVersion_PL as PLincentiveVersion, mvc.originationVersion_LL as originationVersion 
INTO #tmp_version
...
JOIN #tmp_version tpv
    ON ph.marketTicker = tpv.tickerName
JOIN fnm.sec p
...
WHERE 1=1
    ...
    AND sli.version in (SELECT LLincentiveVersion from #tmp_version)
...
DROP TABLE IF EXISTS #FNM_First_Record;
SELECT
    issueId,
    min(asOf) as min_asOf
INTO #FNM_First_Record
FROM #FNM_PoolIncentive
WHERE 1=1
    AND issueId in (SELECT distinct issueId from #FNM_LoanBasedIncentive)
GROUP BY issueId
...
UPDATE #FNM_PoolBasedIncentive inc SET inc.llpa = 0 WHERE inc.marketTicker = 'FNCI';
COMMIT;
...
UPDATE #FNM_PoolBasedIncentive inc 
SET inc.mip= 0
WHERE  inc.mip IS NULL;
-- Update Refinance Incentive
UPDATE #FNM_PoolBasedIncentive inc SET
    conventional_refi_incentive = 100.0 * (CASE WHEN inc.marketTicker = 'FNCT' THEN wac + (pmi_begin / 100.0) - MtgRate - (llpa / 350.0) - (pmi / 100.0)
                                                    ELSE wac + (pmi_begin / 100.0) - MtgRate - (llpa / 400.0) - (pmi / 100.0) END),
...
UPDATE #FNM_PoolBasedIncentive inc SET fha_refi_incentive = NULL WHERE inc.marketTicker in ('FNCI', 'FNCT');
UPDATE #FNM_PoolBasedIncentive SET
    conventional_refi_incentive =0
WHERE conventional_refi_incentive IS NULL
;

UPDATE #FNM_PoolBasedIncentive inc SET 
    refi_incentive = CASE WHEN fha_refi_incentive IS NULL THEN conventional_refi_incentive
                        ELSE CASE WHEN conventional_refi_incentive >= fha_refi_incentive THEN conventional_refi_incentive ELSE fha_refi_incentive END END;
COMMIT;
...
WHERE 1=1
	...
	AND inc.refi_incentive_eligible IS NULL
;
COMMIT;

DROP TABLE IF EXISTS #FNM_IncentiveBias_Total;
SELECT
    avg(refi_incentive_eligible - refi_incentive) as avg_diff
INTO #FNM_IncentiveBias_Total
FROM #FNM_LoanBasedIncentive
WHERE 1=1
    AND refi_incentive_eligible IS NOT NULL
    AND refi_incentive IS NOT NULL
...
UPDATE #FNM_PoolBasedIncentive inc SET
    inc.refi_incentive_eligible = inc.refi_incentive + ib.avg_diff
FROM #FNM_IncentiveBias_Total ib
WHERE 1=1
    AND inc.refi_incentive_eligible IS NULL
;
COMMIT;
...
--SELECT marketTicker, p.asOf, count(1), sum(balance), sum(balance * spi.refi_incentive) / sum(CASE WHEN spi.refi_incentive IS NULL THEN 0.0 ELSE balance END) as wavg_refi_incentive FROM #FNM_PoolIncentive p JOIN scale.FNM_PoolIncentive spi ON p.asof=spi.asof AND p.issueId = spi.issueId  WHERE version = '2.41' GROUP BY marketTicker, p.asOf ORDER BY p.asOf
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for conventional_refi_incentive: %1!', @cnt
            RETURN
        END
...
--       if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for fha_refi_incentive: %1!', @cnt
--            RETURN
--        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for refi_incentive: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for refi_incentive_eligible: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for pmi_begin: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for llpa: %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for pmi: %1!', @cnt
            RETURN
        END
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'Number of FNM Pools with WRONG VALUE for mip: %1!', @cnt
--            RETURN
--        END
----------------------------------------------------------------------------------------------
...
--	 FROM scale.FNM_PoolIncentive
--	 WHERE asOf >= (SELECT asOf from #tmp_asOf)
--  AND asOf <= (select asOf_last from #tmp_asOf)
--  AND asOf <= (select asOf_last from #tmp_asOf)
--  AND version in (SELECT PLincentiveVersion from #tmp_version)
...
WHERE 1=1
	...
	AND spi.asOf <= (select asOf_last from #tmp_asOf)
    AND version in (SELECT PLincentiveVersion from #tmp_version)
;
...
INSERT INTO scale.JobStatus (EndTime, JobName, JobStatus, #LoansBeforeProcess, #LoansUpdated, #LoansAfterProcess)
    SELECT
     getdate(),
    'FNM_Pool_Step 5_Create_Pool_Incentive_Table',
    'SUCCESS', 
    @count_previous,
    @count_current,
