...
VALUES (getdate(),'FHL_Pool_Step 6_Create_Pool_Burnout_Table','START');
COMMIT;
...
SELECT mvc.tickerName, mvc.appType, mvc.incentiveVersion_PL as PLincentiveVersion, mvc.burnoutVersion_PL as PLburnoutVersion, mvc.originationVersion_LL as originationVersion, mvc.burnoutVersion_LL as LLburnoutVersion
INTO #tmp_version
...
WHERE 1=1
    AND spi.version in (select PLincentiveVersion from #tmp_version)
;
COMMIT;
...
WHERE 1=1
    AND slb.version in (select LLburnoutVersion from #tmp_version)
...
JOIN #Mtg30YrRates_monthly_refi mtg
    ON mtg.asOf = t.asOf
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Pools with NULL Value for Burnout: %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_PoolBurnout WHERE burnout < 0.0;

        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Pools with INVALID Value for Burnout: %1!', @cnt
            RETURN
        END
...
--	 FROM scale.FHL_PoolBurnout
--	 WHERE asOf >= (SELECT asOf from #tmp_asOf)
--  AND version in (SELECT PLburnoutVersion from #tmp_version)
...
SELECT
    issueId,
    asOf,
    tpv.PLburnoutVersion AS version,
    burnout
FROM #FHL_PoolBurnout pb
JOIN #tmp_version tpv
ON pb.marketTicker = tpv.tickerName
...
INSERT INTO scale.JobStatus (EndTime, JobName, JobStatus, #LoansBeforeProcess, #LoansUpdated, #LoansAfterProcess)
    SELECT
     getdate(),
    'FHL_Pool_Step 6_Create_Pool_Burnout_Table',
    'SUCCESS', 
    @count_previous,
    @count_current,
