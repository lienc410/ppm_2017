...
SELECT CAST ('2006-01-01' AS DATE) asOf INTO #tmp_asOf
;

-- Before execute the script, need to set the version to run for
DROP TABLE IF EXISTS #tmp_version;
SELECT CAST ('2.60' AS varchar(5)) version INTO #tmp_version
;
...
DROP TABLE IF EXISTS #Mtg30YrRates_monthly_turn;
-- two-month lagged
SELECT
lag1.SeriesName,
asof = lag1.asOf_lag1,
MtgRate = lag1.MtgRate * 0.0 + lag2.MtgRate * 1.0
INTO #Mtg30YrRates_monthly_turn
FROM #Mtg30YrRates_monthly lag1
JOIN #Mtg30YrRates_monthly lag2 ON lag1.asOf_lag1 = lag2.asOf_lag2 AND lag1.SeriesName = lag2.SeriesName
;
COMMIT;

DROP TABLE IF EXISTS #Mtg30YrRates;
SELECT
refi.asof,
refi.SeriesName,
refi.MtgRate AS refiMtgRate,
turn.MtgRate AS turnMtgRate
INTO #Mtg30YrRates
FROM #Mtg30YrRates_monthly_refi refi
JOIN #Mtg30YrRates_monthly_turn turn ON refi.asof = turn.asof AND refi.SeriesName = turn.SeriesName
;
COMMIT;

CREATE LF INDEX series_idx ON #Mtg30YrRates(SeriesName);
CREATE LF INDEX asOf_idx ON #Mtg30YrRates(asOf);
COMMIT;
...
SELECT
    ...
    sle.fha_eligible,
    mr.refiMtgRate as refiMtgRate,
    mr_gn.refiMtgRate as refiMtgRate_GNMA,
    mr.turnMtgRate as turnMtgRate,
    mr_gn.turnMtgRate as turnMtgRate_GNMA,
    ...
    cast(NULL as numeric(8, 4)) as conv_refi_incentive,
    cast(NULL as numeric(8, 4)) as fha_refi_incentive,
    cast(NULL as numeric(8, 4)) as refi_incentive,
    cast(NULL as numeric(8, 4)) as refi_incentive_eligible,
    cast(NULL as numeric(8, 4)) as conv_turn_incentive,
    cast(NULL as numeric(8, 4)) as fha_turn_incentive,
    cast(NULL as numeric(8, 4)) as turn_incentive,
    cast(NULL as numeric(8, 4)) as turn_incentive_eligible
...
JOIN #Mtg30YrRates mr
    ON mr.asOf = clh.asOf
    AND mr.SeriesName = tk.mtgRateSeries
JOIN #Mtg30YrRates mr_gn
...
UPDATE #FHL_LoanIncentive inc SET conv_refi_incentive = origNoteRate + (origPMI / 100.0) - refiMtgRate - (llpa / 4.0) - pmi;

-- Update FHA Refinance Incentive
UPDATE #FHL_LoanIncentive inc SET fha_refi_incentive = origNoteRate + (origPMI / 100.0) - refiMtgRate_GNMA - mip;

-- Update Refinance Incentive
UPDATE #FHL_LoanIncentive inc SET refi_incentive = CASE WHEN conv_refi_incentive >= fha_refi_incentive THEN conv_refi_incentive ELSE fha_refi_incentive END;
...
UPDATE #FHL_LoanIncentive inc SET refi_incentive_eligible = CASE 
        ...
        WHEN HARP_eligible = 0 AND conventional_eligible = 0 AND fha_eligible = 1 THEN fha_refi_incentive
        WHEN (HARP_eligible = 1 OR conventional_eligible = 1) AND fha_eligible = 0 THEN conv_refi_incentive
...
-- Update Conventional Turnover Incentive
UPDATE #FHL_LoanIncentive inc SET conv_turn_incentive = origNoteRate + (origPMI / 100.0) - turnMtgRate - (llpa / 4.0) - pmi;

-- Update FHA Turnover Incentive
UPDATE #FHL_LoanIncentive inc SET fha_turn_incentive = origNoteRate + (origPMI / 100.0) - turnMtgRate_GNMA - mip;

-- Update Turnover Incentive
UPDATE #FHL_LoanIncentive inc SET turn_incentive = CASE WHEN conv_turn_incentive >= fha_turn_incentive THEN conv_turn_incentive ELSE fha_turn_incentive END;

-- Update Turnover Incentive for Eligible Loans
UPDATE #FHL_LoanIncentive inc SET turn_incentive_eligible = CASE 
        WHEN HARP_eligible = 0 AND conventional_eligible = 0 AND fha_eligible = 0 THEN NULL
        WHEN HARP_eligible = 0 AND conventional_eligible = 0 AND fha_eligible = 1 THEN fha_turn_incentive
        WHEN (HARP_eligible = 1 OR conventional_eligible = 1) AND fha_eligible = 0 THEN conv_turn_incentive
        WHEN (HARP_eligible = 1 OR conventional_eligible = 1) AND fha_eligible = 1 THEN turn_incentive
        ELSE NULL 
    END;
COMMIT;
...
DELETE FROM scale.FHL_LoanIncentive_Dev
FROM scale.FHL_LoanIncentive_Dev sli, #FHL_LoanIncentive li
WHERE 1=1
...
INSERT INTO scale.FHL_LoanIncentive_Dev (loanSeqNum, asOf, version, conventional_refi_incentive, fha_refi_incentive, refi_incentive, refi_incentive_eligible, 
                                        currLLPA, currPMI, currMIP, conventional_turn_incentive, fha_turn_incentive, turn_incentive, turn_incentive_eligible)
SELECT
    loanSeqNum,
    asOf,
    @version,
    conv_refi_incentive * 100.0,
    fha_refi_incentive * 100.0,
    refi_incentive * 100.0,
    refi_incentive_eligible * 100.0,
    llpa * 100.0,
    pmi * 100.0,
    mip * 100.0,
    conv_turn_incentive * 100.0,
    fha_turn_incentive * 100.0,
    turn_incentive * 100.0,
    turn_incentive_eligible * 100.0
