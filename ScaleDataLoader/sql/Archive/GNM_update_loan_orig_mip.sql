...
;
INSERT #ticker SELECT 'GNSF', 'GINNIE_30YR';   --Ginne
...
--remove
select distinct(loanType) from #GNM_OrigMIP
WHERE mip_begin > 500.0
--end remove
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'GNM Loans with NULL VALUE for  mip_begin LoanCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'GNM Loans with INVALID RANGE VALUE for  mip_begin LoanCount : %1!', @cnt
            RETURN
        END
