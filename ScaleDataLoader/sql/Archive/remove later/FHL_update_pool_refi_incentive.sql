...
UPDATE #FHL_PoolBasedIncentive inc SET inc.pmi_begin_base = pmi.PMI
...
UPDATE #FHL_PoolBasedIncentive inc SET inc.pmi_begin_loansize = pmi.PMI
...
UPDATE #FHL_PoolBasedIncentive inc SET inc.pmi_begin_occ = (pmi_owner.PMI * inc.Pct_OWNER + pmi_2nd.PMI * inc.Pct_2ND + pmi_inv.PMI * inc.Pct_INV) / 100.0
...
-- Update pmi_end_base
UPDATE #FHL_PoolBasedIncentive inc SET inc.pmi_end_base = pmi.PMI
FROM report.PIV_PMI_Base pmi
WHERE 1=1
    AND LoanPurpose = 'REFI'
...
UPDATE #FHL_PoolBasedIncentive inc SET inc.pmi_end_loansize = pmi.PMI
FROM report.PIV_PMI_LoanSize pmi
WHERE 1=1
    AND inc.asOf >= pmi.startDate and inc.asOf < pmi.endDate 
    AND inc.waocs >= pmi.FICObucketStart and inc.waocs < pmi.FICObucketEnd
...
-- Update pmi_end_occ
UPDATE #FHL_PoolBasedIncentive inc SET inc.pmi_end_occ = (pmi_owner.PMI * inc.Pct_OWNER + pmi_2nd.PMI * inc.Pct_2ND + pmi_inv.PMI * inc.Pct_INV) / 100.0
FROM report.PIV_PMI_Occupancy pmi_owner, report.PIV_PMI_Occupancy pmi_2nd, report.PIV_PMI_Occupancy pmi_inv
WHERE 1=1
    AND pmi_owner.OccType = 'OWNER'
...
-- Update Base LLPA
UPDATE #FHL_PoolBasedIncentive inc SET inc.llpa_base = llpa.LLPA
...
UPDATE #FHL_PoolBasedIncentive inc SET inc.llpa_occ = (llpa_owner.LLPA * inc.Pct_OWNER + llpa_2nd.LLPA * inc.Pct_2ND + llpa_inv.LLPA * inc.Pct_INV) / 100.0
...
-- Update Loan Size
UPDATE #FHL_PoolBasedIncentive inc SET inc.llpa_loansize = llpa.LLPA
...
--;
UPDATE #FHL_PoolBasedIncentive inc SET inc.mip = mip_base.AnnualMIPPoints + mip_base.UpfrontMIPPoints / 12.0
