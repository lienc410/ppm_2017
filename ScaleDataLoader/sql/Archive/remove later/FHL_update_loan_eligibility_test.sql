...
SELECT CAST ('2016-12-01' AS DATE) asOf INTO #tmp_asOf
;
...
SELECT distinct
    ...
    sl.loanSeqNum,
    slh.asOf,
    sl.origFICO, slh.cltv, cl.occType, sl.numberUnits, clh.currentRPB, slh.balance,
...
select 1 - sum(case when conventional_eligible = 0 then balance else 0 end) / sum(balance)
select 1 - sum(case when occType IS NULL then balance else 0 end) / sum(balance)
from #FHL_conventional_eligibility
where 1=1
and asof >= '2009-06-01'
and cltv >= 30
and cltv < 40
--and conventional_eligible = 0

select * from #FHL_conventional_eligibility
...
SELECT
    ...
    CASE 
        ...
        WHEN sl.origFICO >= 580 AND slh.cltv <= 97.5 THEN 1
...
select * from #FHL_conventional_eligibility ce
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Loans with Invalid HARP Eligibile Range LoanCount : %1!', @cnt
            RETURN
        END  
...
INSERT INTO scale.FHL_LoanEligibility (loanSeqNum, asOf, HARP_eligible, conventional_eligible, fha_eligible)
SELECT
    he.loanSeqNum,
    he.asOf,
    he.HARP_eligible,
