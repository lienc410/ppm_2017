...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_LoanHist  WHERE    origLoanSize IS NULL OR origLoanSize <= 0.0 OR origLoanSize > 10000000
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_LoanHist  WHERE   numberUnits IS NULL OR numberUnits <= 0 OR numberUnits > 10
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_LoanHist  WHERE   origFICO IS NULL OR origFICO <= 0.0 OR origFICO > 900
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_LoanHist  WHERE   origLTV IS NULL OR origLTV <= 0.0 OR origLTV > 1000
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_LoanHist  WHERE    origNoteRate IS NULL OR origNoteRate <= 0.0 OR origNoteRate > 20
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_LoanHist  WHERE   miLevel IS NULL OR miLevel < 0.0 OR miLevel > 80
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_LoanHist  WHERE  HPI_orig IS NULL OR HPI_orig < 0.0 OR HPI_orig > 1000
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Loans with INVALID VALUE for  HPA_2YR LoanCount : %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
