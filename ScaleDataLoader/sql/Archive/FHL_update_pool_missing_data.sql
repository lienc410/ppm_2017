...
WHERE 1=1
    ...
    AND f.wac > 0
    AND balance > 0.01
    AND f.asOf NOT IN ('1994-05-01','1997-01-01') -- bad dates ('1994-06-01','1997-02-01') -- old bad data dates
...
DELETE FROM #FHL_PoolHist WHERE marketTicker = 'FGU6' AND asOf IN ('2009-10-01'); -- bad date ('2009-11-01') -- old bad data date
COMMIT;
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for  CLTV PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for balance PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for waolSize PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for waocs PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for waoltv PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for factor PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for wac PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for waclSize PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for waCLTV PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for HPI_orig PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for HPA PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for miLevel PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        SELECT
            @cnt =  count(1)
        FROM #FHL_PoolHist WHERE asof = (SELECT asOf FROM #tmp_asOf)

        SELECT
            @cnt_previous =  count(1)
        FROM scale.FHL_PoolHist WHERE asof = (SELECT dateadd(month, -1, asOf) FROM #tmp_asOf)
        
        if (@cnt - @cnt_previous > 0.1 * @cnt_previous OR @cnt_previous - @cnt > 0.1 * @cnt_previous)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID COUNT compare with previous month. Currnt: %1!; Previous: %2!', @cnt, @cnt_previous
            RETURN
        END 
-- End Tests
...
INSERT INTO scale.FHL_PoolHist (issueId, marketTicker, asOf, balance, originationDate, origLoanSize, origFICO, origLTV, origNoteRate, currLoanSize, cltv, hpa)
SELECT
    issueId,
    marketTicker,
    asOf,
