...
--    cast(NULL as numeric(6, 3)) as Pct_NA,
    ...
    cast(NULL as numeric(6, 3)) as Pct_PIH,
    cast(NULL as numeric(6, 3)) as Pct_RHS,
    cast(NULL as numeric(6, 3)) as Pct_NA_TYPE
...
SELECT
    ...
    sum(Pct_RHS) as Pct_RHS_TYPE,
    sum(Pct_PIH) as Pct_PIH_TYPE,
    sum(Pct_NA_TYPE) as Pct_NA_TYPE
INTO #GNM_Type_Dist
FROM
(
SELECT
    ...
    CASE WHEN state IN ('RHS') THEN percentRpb ELSE 0.0 END as Pct_RHS,
    CASE WHEN state IN ('PIH') THEN percentRpb ELSE 0.0 END as Pct_PIH,
    CASE WHEN state IN ('N/A') THEN percentRpb ELSE 0.0 END as Pct_NA_TYPE
FROM #GNM_PoolDistribution p
...
-- Remove paid off pools
DELETE FROM #GNM_Type_Dist WHERE Pct_FHA_TYPE + Pct_VA_TYPE + Pct_RHS_TYPE + Pct_PIH_TYPE + Pct_NA_TYPE <= 0.01
COMMIT;

UPDATE #GNM_PoolDistribution perf SET perf.Pct_FHA = ty.Pct_FHA_TYPE, perf.Pct_VA = ty.Pct_VA_TYPE, perf.Pct_RHS = ty.Pct_RHS_TYPE, perf.Pct_PIH = ty.Pct_PIH_TYPE, perf.Pct_NA_TYPE = ty.Pct_NA_TYPE
...
UPDATE #GNM_PoolDistribution SET Pct_NA_TYPE = 100.0 - Pct_FHA - Pct_VA - Pct_RHS - Pct_PIH
WHERE 1=1
    AND asOf < '2005-09-01' AND asOf >= '1999-07-01'
    AND Pct_FHA >= 0.0 AND Pct_FHA <= 100.0
    AND Pct_VA >= 0.0 AND Pct_VA <= 100.0
    AND (Pct_NA_TYPE = 0.0 OR Pct_NA_TYPE IS NULL)
    AND 100.0 - Pct_FHA - Pct_VA - Pct_RHS - Pct_PIH >= 0.0
;
COMMIT;
...
SELECT
    ...
    cast(NULL as numeric(12, 2)) as Pct_PIH,
    cast(NULL as numeric(12, 2)) as Pct_RHS,
    cast(NULL as numeric(12, 2)) as Pct_NA_TYPE
INTO #GNM_MinDate_TYPE
FROM #GNM_PoolDistribution p
WHERE p.Pct_FHA IS NOT NULL AND p.Pct_VA IS NOT NULL AND Pct_NA_TYPE IS NOT NULL
GROUP BY issueId
;
COMMIT;

UPDATE #GNM_MinDate_TYPE s SET s.Pct_FHA = d.Pct_FHA, s.Pct_VA = d.Pct_VA,  s.Pct_RHS = d.Pct_RHS, s.Pct_PIH = d.Pct_PIH, s.Pct_NA_TYPE = d.Pct_NA_TYPE
FROM #GNM_PoolDistribution d
WHERE s.issueId = d.issueId
    AND s.asOf = d.asOf
;

UPDATE #GNM_PoolDistribution perf SET perf.Pct_FHA = s.Pct_FHA, perf.Pct_VA = s.Pct_VA, perf.Pct_RHS = s.Pct_RHS, perf.Pct_PIH = s.Pct_PIH, perf.Pct_NA_TYPE = s.Pct_NA_TYPE
FROM #GNM_MinDate_TYPE s
WHERE perf.issueId = s.issueId 
    AND (perf.Pct_FHA IS NULL OR perf.Pct_VA IS NULL OR perf.Pct_RHS IS NULL OR perf.Pct_PIH IS NULL)
    AND perf.asOf <= s.asOf
;
...
SELECT
    ...
    sum(balance * Pct_RHS) / sum(balance) as WAVG_RHS,
    sum(balance * Pct_PIH) / sum(balance) as WAVG_PIH,
    sum(balance * Pct_NA_TYPE) / sum(balance) as WAVG_NA_TYPE,
    sum(balance) as sum_ls
INTO #GNM_OriginationDate_TYPE
FROM #GNM_PoolDistribution
WHERE Pct_FHA IS NOT NULL AND Pct_VA IS NOT NULL AND Pct_RHS IS NOT NULL AND Pct_PIH IS NOT NULL AND Pct_NA_TYPE IS NOT NULL
GROUP BY origYear
HAVING sum_ls > 100000000
;
COMMIT;

UPDATE #GNM_PoolDistribution perf SET perf.Pct_FHA = type.WAVG_FHA, perf.Pct_VA = type.WAVG_VA, perf.Pct_RHS = type.WAVG_RHS, perf.Pct_PIH = type.WAVG_PIH, perf.Pct_NA_TYPE = type.WAVG_NA_TYPE
...
UPDATE #GNM_PoolDistribution SET Pct_RHS = 100.0
WHERE Pct_RHS > 100.0
;

UPDATE #GNM_PoolDistribution SET Pct_RHS = 0.0
WHERE Pct_RHS < 0.0
;

UPDATE #GNM_PoolDistribution SET Pct_PIH = 100.0
WHERE Pct_PIH > 100.0
;

UPDATE #GNM_PoolDistribution SET Pct_PIH = 0.0
WHERE Pct_PIH < 0.0

UPDATE #GNM_PoolDistribution SET Pct_NA_TYPE = 100.0
WHERE Pct_NA_TYPE > 100.0
;

UPDATE #GNM_PoolDistribution SET Pct_NA_TYPE = 0.0
WHERE Pct_NA_TYPE < 0.0
;

COMMIT;

UPDATE #GNM_PoolDistribution SET Pct_FHA = avg_FHA, Pct_VA = avg_VA, Pct_RHS = avg_RHS, Pct_PIH = avg_PIH, Pct_NA_TYPE = avg_NA_TYPE
FROM (SELECT avg(Pct_FHA) as avg_FHA, avg(Pct_VA) as avg_VA, avg(Pct_RHS) as avg_RHS, avg(Pct_PIH) as avg_PIH, avg(Pct_NA_TYPE) as avg_NA_TYPE FROM #GNM_PoolDistribution WHERE Pct_FHA IS NOT NULL) t
WHERE Pct_FHA IS NULL OR Pct_VA IS NULL OR Pct_RHS IS NULL OR Pct_PIH IS NULL OR Pct_NA_TYPE IS NULL
;
COMMIT;

-- Normalize to 100%
UPDATE #GNM_PoolDistribution SET Pct_FHA = Pct_FHA - ((Pct_FHA + Pct_VA + Pct_RHS + Pct_PIH + Pct_NA_TYPE - 100.0) / 5.0), 
    Pct_VA = Pct_VA - ((Pct_FHA + Pct_VA + Pct_RHS + Pct_PIH + Pct_NA_TYPE - 100.0) / 5.0),
    Pct_RHS = Pct_RHS - ((Pct_FHA + Pct_VA + Pct_RHS + Pct_PIH + Pct_NA_TYPE - 100.0) / 5.0),
    Pct_PIH = Pct_PIH - ((Pct_FHA + Pct_VA + Pct_RHS + Pct_PIH + Pct_NA_TYPE - 100.0) / 5.0),
    Pct_NA_TYPE = Pct_NA_TYPE - ((Pct_FHA + Pct_VA + Pct_RHS + Pct_PIH + Pct_NA_TYPE - 100.0) / 5.0)
WHERE abs(Pct_FHA + Pct_VA + Pct_RHS + Pct_PIH + Pct_NA_TYPE - 100.0) > 0.05
;
COMMIT;
...
DELETE FROM scale.GNM_PoolDistribution_v2_00
...
INSERT INTO scale.GNM_PoolDistribution_v2_00 (issueId, asOf, percentPURP_PURCH, percentPURP_REFI, percentCHANNEL_RETAIL, percentCHANNEL_BROKER, percentCHANNEL_CORRES, percentUNIT_SINGLE, percentUNIT_MULTIPLE, percentCURRENT, percentDELQ30plus, percentDELQ60plus, percentDELQ90plus, percentTYPE_FHA, percentTYPE_VA, percentTYPE_RHS, percentTYPE_PIH, percentTYPE_NA)
SELECT
    issueId, 
    asOf,  
    Pct_PURCH, 
