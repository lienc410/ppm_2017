...
UPDATE #FHL_LoanIncentive inc SET conv_incentive = origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 4.0) - pmi;
...
----------------------------------------------------------------------------------------------
        ...
        SELECT
            @cnt =  count(1)
        FROM #FHL_LoanIncentive WHERE asof = (SELECT asOf FROM #tmp_asOf)

        SELECT
            @cnt_previous =  count(1)
        FROM scale.FHL_LoanIncentive WHERE asof = (SELECT dateadd(month, -1, asOf) FROM #tmp_asOf) AND version = (select  version from #tmp_version)
...
INSERT INTO scale.FHL_LoanIncentive (loanSeqNum, asOf, version, conventional_refi_incentive, fha_refi_incentive, refi_incentive_eligible, currLLPA, currPMI, currMIP)
SELECT
    loanSeqNum,
    asOf,
    @version,
