...
(
    ...
    SELECT loanSeqNum, asOf, schambalance, currentRPB FROM fnm.PIV_Derived_LoanHist
) clh
...
UPDATE #FNM_LoanIncentive inc SET conv_incentive = origNoteRate + (origPMI / 100.0) - MtgRate - (llpa / 4.0) - pmi;

-- Update FHA Refinance Incentive
UPDATE #FNM_LoanIncentive inc SET fha_incentive = origNoteRate + (origPMI / 100.0) - MtgRate_GNMA - mip;
...
UPDATE #FNM_LoanIncentive inc SET refi_incentive_eligible = CASE 
        ...
        WHEN (HARP_eligible = 1 OR conventional_eligible = 1) AND fha_eligible = 1 THEN 
            CASE WHEN conv_incentive >= fha_incentive THEN conv_incentive 
            ELSE fha_incentive 
            END
        ELSE NULL 
    END;
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with NULL conv_incentive LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with NULL fha_incentive LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with NULL llpa LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with NULL pmi LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with NULL mip LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with Invalid conv_incentive Range LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with Invalid fha_incentive Range LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with Invalid llpa Range LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with Invalid pmi Range LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        if (@cnt > 0)
--        BEGIN
--            RAISERROR 99999 'FNM Loans with Invalid mip Range LoanCount : %1!', @cnt
--            RETURN
--        END
------------------------------------------------------------------------------------------------
...
--        SELECT
--            @cnt =  count(1)
--        FROM #FNM_LoanIncentive WHERE asof = (SELECT asOf FROM #tmp_asOf)

--        SELECT
--            @cnt_previous =  count(1)
--        FROM scale.FNM_LoanIncentive WHERE asof = (SELECT dateadd(month, -1, asOf) FROM #tmp_asOf) AND version = (select  version from #tmp_version)
        
--        if (@cnt - @cnt_previous > 0.1 * @cnt_previous OR @cnt_previous - @cnt > 0.1 * @cnt_previous)
--        BEGIN
--            RAISERROR 99999 'FNM LoanIncentive with INVALID COUNT compare with previous month. Current: %1!; Previous: %2!', @cnt, @cnt_previous
--            RETURN
--        END 
---- End Tests
...
INSERT INTO scale.FNM_LoanIncentive (loanSeqNum, asOf, version, conventional_refi_incentive, fha_refi_incentive, refi_incentive_eligible, currLLPA, currPMI, currMIP)
SELECT
    loanSeqNum,
    asOf,
    @version,
