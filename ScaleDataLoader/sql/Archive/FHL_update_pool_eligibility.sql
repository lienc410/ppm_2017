...
--DROP TABLE IF EXISTS #FHL_MIN_RefiEligibility;
--SELECT
--    issueId,
--    min(asOf) as min_asOf,
--    cast(NULL as numeric(6, 3)) as earliest_refi_eligible
--INTO #FHL_MIN_RefiEligibility
--FROM #FHL_PoolEligibility
--WHERE 1=1
--    AND refi_eligible IS NOT NULL
--    AND issueId in (SELECT distinct issueId from #FHL_Loan_RefiEligibility)
--GROUP BY issueId
--;
--COMMIT;

--UPDATE #FHL_MIN_RefiEligibility mre SET mre.earliest_refi_eligible = pe.refi_eligible
--FROM #FHL_PoolEligibility pe
--WHERE mre.issueId = pe.issueId
--    AND mre.min_asOf = pe.asOf
--;

--UPDATE #FHL_PoolEligibility pe SET pe.refi_eligible = mre.earliest_refi_eligible
--FROM #FHL_MIN_RefiEligibility mre
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for HARP_eligible PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for conventional_eligible PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for fha_eligible PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with INVALID VALUE for refi_eligible PoolCount : %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        ...
        SELECT
            @cnt =  count(1)
        FROM #FHL_PoolEligibility WHERE asof = (SELECT asOf FROM #tmp_asOf)

        SELECT
            @cnt_previous =  count(1)
        FROM scale.FHL_PoolEligibility WHERE asof = (SELECT dateadd(month, -1, asOf) FROM #tmp_asOf)
        
        if (@cnt - @cnt_previous > 0.1 * @cnt_previous OR @cnt_previous - @cnt > 0.1 * @cnt_previous)
        BEGIN
            RAISERROR 99999 'FHL PoolEligibility with INVALID COUNT compare with previous month. Currnt: %1!; Previous: %2!', @cnt, @cnt_previous
            RETURN
        END 
-- End Tests
