...
-- Create Average MI Level stimates using FHL Pool Data
...
-- Create Average Throught History Mi Level stimates using FHL Pool Data
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for  CLTV PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for balance PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for waolSize PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for waocs PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for waoltv PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for factor PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for wac PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for waclSize PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for waCLTV PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for HPI_orig PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for HPA PoolCount : %1!', @cnt
            RETURN
        END
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FNM Pools with INVALID VALUE for miLevel PoolCount : %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        ...
        SELECT
            @cnt =  count(1)
        FROM #FNM_PoolHist WHERE asof = (SELECT asOf FROM #tmp_asOf)

        SELECT
            @cnt_previous =  count(1)
        FROM scale.FNM_PoolHist WHERE asof = (SELECT dateadd(month, -1, asOf) FROM #tmp_asOf)
        
        if (@cnt - @cnt_previous > 0.1 * @cnt_previous OR @cnt_previous - @cnt > 0.1 * @cnt_previous)
        BEGIN
            RAISERROR 99999 'FNM PoolHist with INVALID COUNT compare with previous month. Currnt: %1!; Previous: %2!', @cnt, @cnt_previous
            RETURN
        END 
-- End Tests
