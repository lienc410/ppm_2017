...
SELECT CAST ('1.00' AS varchar(5)) version INTO #tmp_version
;
...
------------------------------------------------------------
DROP TABLE IF EXISTS #GNM_LoanBasedBurnout;
SELECT
    pb.issueId,
    pb.asOf,
    sum(slb.burnout * slh.schamBalance) / sum(CASE WHEN slb.burnout IS NULL THEN 0.0 ELSE slh.schamBalance END) as wavg_burnout
INTO #GNM_LoanBasedBurnout
FROM #GNM_PoolBurnout pb
JOIN gnm.PIV_Loan cl
    ON pb.issueId = cl.issueId
JOIN scale.GNM_Loan sl
...
WHERE 1=1
    ...
    AND slb.burnout IS NOT NULL
GROUP BY pb.issueId, pb.asOf
HAVING sum(slh.schamBalance) > 0.1
;
COMMIT;
...
FROM #GNM_LoanBasedBurnout lb
WHERE pb.issueId = lb.issueId
    AND pb.asOf = lb.asOf
;

UPDATE #GNM_PoolBurnout pb SET has_loan_data = 1
WHERE pb.issueId IN (SELECT distinct issueId FROM #GNM_LoanBasedBurnout WHERE wavg_burnout IS NOT NULL)
COMMIT;
...
UPDATE #GNM_PoolBurnout pb SET burnout = llb.max_burnout 
    + 100.0 * log(CASE WHEN origNoteRate / MtgRate  <= 1.0 
                       THEN 1.0 ELSE origNoteRate / MtgRate 
                  END)
FROM #GNM_LoanLatestBurnout llb, #GNM_PoolLastDate pld
WHERE 1=1
...
--------------------------------------------------------------
DROP TABLE IF EXISTS #GNM_LoanRelativeIncentive;
SELECT
    issueId,
    asOf,
    origNoteRate / MtgRate as relative_incentive
INTO #GNM_LoanRelativeIncentive
FROM #GNM_PoolBurnout
WHERE 1=1
    AND has_loan_data = 1
    AND burnout IS NULL
...
------------------------------------------------------------
DROP TABLE IF EXISTS #GNM_EarliestLoanBasedBurnout;
SELECT  
    t0.issueId,
    t0.asOf as asOf,
    sum(log(CASE 
                ...
                END)) as burnout
INTO #GNM_EarliestLoanBasedBurnout
FROM #GNM_LoanRelativeIncentive t0
JOIN #GNM_LoanRelativeIncentive t1
...
FROM #GNM_EarliestLoanBasedBurnout elb
...
----------------------------------------------------------------------------------
DROP TABLE IF EXISTS #GNM_PoolRelativeIncentive;
SELECT
    issueId,
    asOf,
    origNoteRate / MtgRate as relative_incentive
INTO #GNM_PoolRelativeIncentive
FROM #GNM_PoolBurnout
WHERE 1=1
    AND burnout IS NULL
    AND has_loan_data = 0
...
CREATE INDEX issueId_idx ON #GNM_PoolRelativeIncentive(issueId);
CREATE INDEX asOf_idx ON #GNM_PoolRelativeIncentive(asOf);
COMMIT;

------------------------------------------------------------
-- Step 2: Calculate Burnout from Relative Incentive
------------------------------------------------------------
DROP TABLE IF EXISTS #GNM_PoolBasedBurnout;
SELECT  
    t0.issueId,
    t0.asOf as asOf,
    sum(log(CASE 
                    ...
                    ELSE t1.relative_incentive
                END)) as burnout
INTO #GNM_PoolBasedBurnout
FROM #GNM_PoolRelativeIncentive t0
JOIN #GNM_PoolRelativeIncentive t1
    ON t0.issueId = t1.issueId
    AND t0.asOf >= t1.asOf
WHERE 1=1
GROUP BY t0.issueId, t0.asOf
...
------------------------------------------------------------
DROP TABLE IF EXISTS #GNM_Earliest;
SELECT
    pbb.issueId,
    o.first_asOf,
...
INTO #GNM_Earliest
FROM #GNM_PoolBasedBurnout pbb
JOIN #GNM_PoolBurnout pb
...
DROP TABLE IF EXISTS #GNM_MissingHist;
SELECT 
    e.issueId, 
    t.asOf, 
    e.origNoteRate,
    mtg.MtgRate
INTO #GNM_MissingHist
FROM #GNM_Earliest e
JOIN #unique_dates t
...
CREATE INDEX issueId_idx ON #GNM_MissingHist(issueId);
CREATE INDEX asOf_idx ON #GNM_MissingHist(asOf);
COMMIT;

-- Historical Burnout computation
-- Simple Calculation involving MortgageRate and Note Rate
DROP TABLE IF EXISTS #GNM_PoolBurnoutHist;
SELECT 
    t0.issueId,
    t0.asOf,
    sum(log(CASE WHEN t1.origNoteRate / t1.MtgRate <= 1 THEN 1 ELSE t1.origNoteRate / t1.MtgRate END)) as burnout
INTO #GNM_PoolBurnoutHist
FROM #GNM_MissingHist t0
JOIN #GNM_MissingHist t1
    ON t0.issueId = t1.issueId
    AND t0.asOf >= t1.asOf
GROUP BY t0.issueId, t0.asOf
;
...
UPDATE #GNM_Earliest e SET e.burnout = bh.burnout
FROM #GNM_PoolBurnoutHist bh
WHERE bh.issueId = e.issueId
    AND bh.asOf = e.first_asOf;
COMMIT;

-- Update Burnout with Historical Burnout
UPDATE #GNM_PoolBasedBurnout b SET b.burnout = b.burnout + e.burnout
FROM #GNM_Earliest e
WHERE b.issueId = e.issueId
;
COMMIT;

-- Update GNM_PoolBurnout Table
UPDATE #GNM_PoolBurnout pb SET pb.burnout = 100.0 * pbb.burnout
FROM #GNM_PoolBasedBurnout pbb
...
--SELECT count(1) FROM #GNM_PoolBurnout WHERE burnout IS NULL OR burnout < 0.0 OR burnout > 25000;

-- Time Series Tests
-- History for: burnout
--SELECT marketTicker, originationDate, count(1), sum(balance), sum(balance * burnout) / sum(CASE WHEN burnout IS NULL THEN 0.0 ELSE balance END) as wavg_burnout FROM #GNM_PoolBurnout GROUP BY marketTicker, originationDate ORDER BY originationDate
--SELECT marketTicker, asOf, count(1), sum(balance), sum(balance * burnout) / sum(CASE WHEN burnout IS NULL THEN 0.0 ELSE balance END) as wavg_burnout FROM #GNM_PoolBurnout GROUP BY marketTicker, asOf ORDER BY asOf



-- Load Data into Scale Burnout Table
DELETE FROM scale.GNM_PoolBurnout
FROM scale.GNM_PoolBurnout spb, #GNM_PoolBurnout pb
WHERE 1=1
...
INSERT INTO scale.GNM_PoolBurnout (issueId, asOf, version, burnout)
SELECT
    issueId,
    asOf,
    @version,
