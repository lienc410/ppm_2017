...
DROP TABLE IF EXISTS #FHL_PoolBurnout;
SELECT
    ph.marketTicker,
    ph.issueId,
    ph.asOf,
...
WHERE 1=1
    AND spi.version = '2.50' --(select version from #tmp_version)
;
COMMIT;

CREATE HG INDEX issueId_idx ON #FHL_PoolBurnout(issueId);
CREATE LF INDEX asOf_idx ON #FHL_PoolBurnout(asOf);
CREATE LF INDEX ticker_idx ON #FHL_PoolBurnout(marketTicker);
COMMIT;
...
DROP TABLE IF EXISTS #FHL_PoolIncentive_LoanSize;
SELECT 
    tpi.issueId,
    tpi.asOf,
    currLoanSize,
--    CASE WHEN tpi.conventional_incentive * sph.currLoanSize / 175000 > 200 THEN 200 ELSE tpi.conventional_incentive * sph.currLoanSize / 175000 END   AS conventional_incentive,
--    CASE WHEN tpi.fha_incentive * sph.currLoanSize / 175000          > 200 THEN 200 ELSE tpi.fha_incentive * sph.currLoanSize / 175000 END            AS fha_incentive,
...
INTO #FHL_PoolIncentive_LoanSize
FROM #FHL_PoolIncentive tpi
JOIN scale.FHL_PoolHist sph ON tpi.issueId = sph.issueId AND tpi.asOf = sph.asOf
;
COMMIT;

CREATE HG INDEX issueId_idx ON #FHL_PoolIncentive_LoanSize(issueId);
CREATE LF INDEX asOf_idx ON #FHL_PoolIncentive_LoanSize(asOf);
COMMIT;
...
UPDATE #FHL_PoolBurnout pb SET burnout = llb.max_burnout + (CASE WHEN incentive <= 0.0 THEN 0.0 ELSE incentive END)
FROM #FHL_LoanLatestBurnout llb, #FHL_PoolLastDate pld, #FHL_PoolIncentive_LoanSize pils
WHERE 1=1
...
FROM #FHL_PoolIncentive_LoanSize t0
JOIN #FHL_PoolIncentive_LoanSize t1
...
CREATE HG INDEX issueId_idx ON #FHL_MissingHist(issueId);
CREATE LF INDEX asOf_idx ON #FHL_MissingHist(asOf);
COMMIT;
...
UPDATE #FHL_PoolBurnout pb SET pb.burnout = pbb.burnout
FROM #FHL_PoolBasedBurnout pbb
...
-- Update FHL_PoolBurnout Table for empty cells
UPDATE #FHL_PoolBurnout pb SET pb.burnout = 0.0
...
----------------------------------------------------------------------------------------------
        ...
        SELECT
            @cnt =  count(1)
        FROM #FHL_PoolBurnout WHERE burnout IS NULL

        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with NULL Burnout LoanCount : %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        SELECT
            @cnt =  count(1)
        FROM #FHL_PoolBurnout WHERE burnout < 0.0; --OR burnout > 25000;

        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'FHL Pools with Invalid burnout Range LoanCount : %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        ...
        SELECT
            @cnt =  count(1)
        FROM #FHL_PoolBurnout WHERE asof = (SELECT asOf FROM #tmp_asOf  where asOf>'20060901')

        SELECT
            @cnt_previous =  count(1)
        FROM scale.FHL_PoolBurnout WHERE asof = (SELECT dateadd(month, -1, asOf) FROM #tmp_asOf  where asOf>'20060901') AND version = (select  version from #tmp_version)
        
        if (@cnt - @cnt_previous > 0.1 * @cnt_previous OR @cnt_previous - @cnt > 0.1 * @cnt_previous)
        BEGIN
            RAISERROR 99999 'FHL PoolBurnout with INVALID COUNT compare with previous month. Current: %1!; Previous: %2!', @cnt, @cnt_previous
            RETURN
        END 
-- End Tests
