...
JOIN 
    (select loanseqnum, asof, refi_incentive_eligible, refi_incentive_eligible_fixed_cost, version from scale.FHL_LoanIncentive 
	union
	 select loanseqnum, asof, refi_incentive_eligible, refi_incentive_eligible_fixed_cost, version from scale.fhl_missing_hist)
	sli
...
-- Set Incentive = 0 where loanAge = -1
UPDATE #FHL_LoanIncentive tli
SET incentive = 0, incentive_fc = 0
FROM FHL.PIV_LoanHist clh
...
delete from #FHL_LoanBurnout where asof<(select asOf_start from #tmp_asOf);
commit;
...
UPDATE #FHL_LoanBurnout b SET b.burnout_fc = 0.0
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Loans with NULL Value for Burnout: %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FHL Loans with INVALID Value for Burnout: %1!', @cnt
            RETURN
        END
----------------------------------------------------------------------------------------------
...
INSERT INTO scale.FHL_LoanBurnout (loanSeqNum, asOf, version, burnout, burnout_fixed_cost)
SELECT
    loanSeqNum,
    asOf,
    version,
...
--INSERT INTO scale.JobStatus (EndTime, JobName, JobStatus, #LoansBeforeProcess, #LoansUpdated, #LoansAfterProcess)
 --   SELECT
 --    getdate(),
  --  'FHL_Loan_Step 7_Create_Loan_Burnout_Table', 
  --  'SUCCESS', 
 --   @count_previous,
 --   @count_current,
