...
-- Update PMI
UPDATE #FNM_conventional_eligibility ce
...
--Calculate REFIEligibleCLTVThreshold
DROP TABLE IF EXISTS #FNM_refi_eligibility_threshold;
SELECT
    sl.loanSeqNum,
    sl.origFICO,
    sl.numberUnits,
...
INTO #FNM_refi_eligibility_threshold
FROM fnm.PIV_Loan cl
JOIN scale.FNM_Loan_Dev sl
    ON cl.loanSeqNum = sl.loanSeqNum
JOIN #ticker tk
    ON sl.marketTicker = tk.tickerName
    AND sl.version IN (select originationVersion from #tmp_version)
    --If the rows are less then it deletes records from scale.FNM_LoanEligibility
    --AND sl.originationDate > '2009-06-01'
;
COMMIT;

Update #FNM_refi_eligibility_threshold Set REFIEligibleCLTVThreshold = Case
    WHEN ConventionalEligibleCLTV > FHAEligibleCLTV THEN ConventionalEligibleCLTV
    ELSE FHAEligibleCLTV
END;
COMMIT;

Update #FNM_refi_eligibility_threshold ret
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)  
        BEGIN
            RAISERROR 99999 'Number of FNM Loans with NULL Value for HARP Eligibility: %1!', @cnt
            RETURN
        END  
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)  
        BEGIN
            RAISERROR 99999 'Number of FNM Loans with NULL Value for conventional Eligibility: %1!', @cnt
            RETURN
        END  
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)  
        BEGIN
            RAISERROR 99999 'Number of FNM Loans with NULL Value for fha Eligibility: %1!', @cnt
            RETURN
		END  
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)  
        BEGIN
            RAISERROR 99999 'Number of FNM Loans with INVALID Value for FHA Eligibility for INVESTOR / 2ND: %1!', @cnt
            RETURN
        END 
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)
        BEGIN
            RAISERROR 99999 'Number of FNM Loans with INVALID HARP Eligibility: %1!', @cnt
            RETURN
        END  
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0)  
        BEGIN
            RAISERROR 99999 'Number of FNM Loans with INVALID conventional Eligibility: %1!', @cnt
            RETURN
        END 
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0) 
        BEGIN
            RAISERROR 99999 'Number of FNM Loans with INVALID fha Eligibility: %1!', @cnt
            RETURN
        END 
----------------------------------------------------------------------------------------------
-- Check no duplicates
----------------------------------------------------------------------------------------------
        ...
        if (@cnt > 0) 
        BEGIN
            RAISERROR 99999 'Number of FHL Loans with Duplicates: %1!', @cnt
            RETURN
        END 
----------------------------------------------------------------------------------------------
...
INSERT INTO scale.FNM_LoanEligibility (loanSeqNum, asOf, HARP_eligible, conventional_eligible, fha_eligible, REFIEligibleCLTVThreshold)
SELECT
    he.loanSeqNum,
    he.asOf,
    he.HARP_eligible,
...
JOIN #FNM_fha_eligibility fe
    ...
    AND fe.asOf = ce.asOf
JOIN #FNM_refi_eligibility_threshold ret
