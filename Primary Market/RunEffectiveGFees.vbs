...
' >> Example - cscript RunEffectiveGFees /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunEffectiveGFees /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "7.Upfront Loan Level Price Adjustments Master.xlsm", 0, True)  ' open read-only
 ...
 'xlApp.visible = true	
	...
	' run the report
	xlApp.run "RunEffectiveGFeeReport"
