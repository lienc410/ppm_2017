...
' >> Example - cscript RunApplicationActivityReport /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunApplicationActivityReport /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "9.Single Family Application Activity Master.xlsm", 0, True)  ' open read-only
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunApplicationActivityReport")
 ...
 'xlApp.visible = true	
	...
	' run the report
	xlApp.run "RunApplicationActivityReportBatch"
