...
' >> Example - cscript RunSpecifiedPoolReportFreddie /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunSpecifiedPoolReportFreddie /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "8.Specified Pools Master - Freddie Mac.xlsm", 0, True)  ' open read-only
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunSpecifiedPoolReportFreddie")
 ...
 'xlApp.visible = true	
	...
	' run the report
	xlApp.run "RunSpecifiedPoolDailyReportBatch"
