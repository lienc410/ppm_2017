...
' >> Example - cscript RunRefinancibleUniverseReport /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunRefinancibleUniverseReport /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	'Set xlBook = xlApp.Workbooks.Open(masterPath & "11.Refinanceable Percentage Master.xlsm", 0, True)  ' open read-only
	Set xlBook = xlApp.Workbooks.Open(masterPath & "11.Refinanceable Percentage Master.xlsm", 0, False)  ' open read-write
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunRefinancibleUniverseReport")
