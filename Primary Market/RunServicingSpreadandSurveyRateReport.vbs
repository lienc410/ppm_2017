...
' >> Example - cscript RunServicingSpreadandSurveyRateReport /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunServicingSpreadandSurveyRateReport /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "3.Servicing Spread and Survey Rate Master.xlsm", 0, True)  ' open read-only
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunServicingSpreadandSurveyRateReport")
 ...
 'xlApp.visible = true	
	...
	' run the report
	xlApp.run "RunServicingSpreadandSurveyRateReportBatch"
