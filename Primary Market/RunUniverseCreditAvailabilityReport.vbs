...
' >> Example - cscript RunUniverseCreditAvailabilityReport /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunUniverseCreditAvailabilityReport /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "14.Universe Credit Availability Master.xlsm", 0, True)  ' open read-only
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunUniverseCreditAvailabilityReport")
 ...
 'xlApp.visible = true	
	...
	' run the report
	xlApp.run "RunCreditAvailabilityReportBatch"
