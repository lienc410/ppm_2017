...
' >> Example - cscript RunMarketConcentrationReport_NonBankLender /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunMarketConcentrationReport_NonBankLender /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "5.Market Concentration Master - Non-Bank Lenders.xlsm", 0, True)  ' open read-only
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunMarketConcentrationReport_NonBankLender")
	WScript.stdout.WriteLine("Close Date = " & closeDate)
	WScript.stdout.WriteLine("Output Folder = " & pdfReportDir)
	'WScript.stdout.WriteLine("Output Report Name = " & PDFReportName)
 ...
 'xlApp.visible = true	
	...
	' run the report
	xlApp.run "RunMarketShareDailyReportBatch"
