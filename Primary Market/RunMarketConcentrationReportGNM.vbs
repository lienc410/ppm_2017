...
' >> Example - cscript RunMarketConcentrationReportGNM /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunMarketConcentrationReportGNM /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "5.Market Concentration Master - GNM.xlsm", 0, True)  ' open read-only
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunMarketConcentrationReportGNM")
