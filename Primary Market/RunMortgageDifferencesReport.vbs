...
' >> Example - cscript RunMortgageDifferencesReport /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunMortgageDifferencesReport /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "13.Rate Difference between Big Lenders and Small Lenders.xlsm", 0, True)  ' open read-only
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunMortgageDifferencesReport")
 ...
 'xlApp.visible = true	
	...
	' run the report
	xlApp.run "RunMorgageRateReportBatch"
