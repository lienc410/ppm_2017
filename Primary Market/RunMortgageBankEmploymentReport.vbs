...
' >> Example - cscript RunMortgageBankEmploymentReport /date:12/4/13 /out:c:\tmp\ /env:PROD

if WScript.Arguments.Count < 2 then
	msgbox "Please specify => Syntax - RunMortgageBankEmploymentReport /date:<mm/dd/yy> /out:<PDF Report File Directory> /env:<PROD | DEV>"
else
	RunReport
end if
...
Sub RunReport()
	...
	Set xlBook = xlApp.Workbooks.Open(masterPath & "12.Mortgage Bank Employment.xlsm", 0, True)  ' open read-only
	
	WScript.stdout.WriteLine("--------- Input Parameters ---------")
	WScript.stdout.WriteLine("Report: RunMortgageBankEmploymentReport")
 ...
 'xlApp.visible = true	
	...
	' run the report
	xlApp.run "RunMortgageBankEmploymentReportBatch"
